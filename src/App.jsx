...
const TProfil = T1;
const TM = { case1:T1, case2:T2, shopify:TS, rode:TR, fastgoodcuisine:TFGC, fgcmarque:TFGCMarque, toinelag:TToinelag, bilantoinelag:TToinelag, cyrilmp4:TC, garmin:TGarmin, otacospepe:TOtacosPepe, profil:TProfil };
...
const DeckMotif = ({deck}) => {
  ...
  if(deck==="toinelag"||deck==="bilantoinelag") return <div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none"}}>{[...Array(26)].map((_,i)=><div key={i} style={{position:"absolute",left:`${(i*37+11)%100}%`,top:`${(i*51+9)%100}%`,width:3+(i%4),height:3+(i%4),borderRadius:"50%",background:i%3===0?"rgba(30,116,232,.14)":"rgba(255,255,255,.55)"}}/>)}</div>;
...
const SV={
  ...
  bilantoinelag:{i:{opacity:0,y:32},a:{opacity:1,y:0},e:{opacity:0,y:-26},t:{type:"spring",stiffness:260,damping:26}},
...
const bilMai = (f) => pu(`/bilan-toinelag-mai2026/${f}`);
const SBilanToinelagMai2026 = [
{title:"Toinelag × FAR — Mai 2026",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",alignItems:"center",gap:40,flexWrap:"wrap",minHeight:"56vh"}}><div style={{flex:"1 1 260px",display:"flex",flexDirection:"column",gap:10}}><div style={{...se,fontSize:52,fontWeight:900,color:t.c,lineHeight:1}}>TOINELAG</div><div style={{...se,fontSize:32,fontWeight:800,color:t.a,lineHeight:1}}>×</div><div style={{...se,fontSize:48,fontWeight:900,color:t.c,lineHeight:1}}>FAR</div><div style={{...mo,fontSize:14,letterSpacing:5,color:t.d,marginTop:6}}>MAI 2026</div></div><div style={{flex:"1.1 1 300px",display:"flex",justifyContent:"center"}}><div style={{maxWidth:440,width:"100%",borderRadius:18,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,background:t.card}}><img src={bilMai("image23.png")} alt="" style={{width:"100%",display:"block",objectFit:"contain"}}/></div></div></div></div>},
{title:"Sommaire",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",gap:28,alignItems:"stretch",flexWrap:"wrap"}}><div style={{flex:"1.1 1 340px"}}><Tg t={t}>SOMMAIRE</Tg><Hl t={t} s={{fontSize:34,marginBottom:18}}>Bilan mai 2026</Hl><div style={{display:"flex",flexDirection:"column",gap:12}}>{["1 — Quatre très beaux succès","2 — De nouveaux process mis en place","3 — Des opportunités non-abouties","4 — Un recrutement qui a transformé l'exécution","5 — Autres ?"].map((x,i)=><div key={i} style={{...sa,fontSize:17,fontWeight:600,color:t.c,padding:"12px 16px",borderRadius:12,background:t.card,border:`1px solid ${t.brd}`,boxShadow:t.cS,lineHeight:1.35}}>{x}</div>)}</div></div><div style={{flex:"0.9 1 280px",minHeight:280,borderRadius:18,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,background:t.card}}><img src={bilMai("image11.jpg")} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block",minHeight:280}}/></div></div></div>},
{title:"Quatre très beaux succès",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",gap:24,flexWrap:"wrap",alignItems:"flex-start"}}><div style={{flex:"0.95 1 300px"}}><Tg t={t}>1 — SUCCÈS</Tg><Hl t={t} s={{fontSize:32,lineHeight:1.15,marginBottom:12}}>1- Quatre très beaux succès</Hl><div style={{...se,fontSize:28,fontWeight:900,color:t.a}}>CA signé : 147K€ HT</div></div><div style={{flex:"1.15 1 360px",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{["image12.jpg","image1.png","image10.png","image5.png","image21.png","image16.png"].map((f)=><div key={f} style={{borderRadius:12,background:t.card,border:`1px solid ${t.brd}`,padding:8,display:"flex",alignItems:"center",justifyContent:"center",minHeight:100}}><img src={bilMai(f)} alt="" style={{maxWidth:"100%",maxHeight:120,objectFit:"contain"}}/></div>)}</div></div></div>},
{title:"Ce qui marche",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",gap:24,flexWrap:"wrap",alignItems:"stretch"}}><div style={{flex:"1.15 1 380px"}}><Tg t={t}>CE QUI MARCHE</Tg><Hl t={t} s={{fontSize:28,marginBottom:16}}>CE QUI MARCHE :</Hl><div style={{display:"flex",flexDirection:"column",gap:14}}>{[{n:"1.",t:"Respect des délais & dates de publication",d:"Les opérations ont pu sortir dans les temps, ce qui a renforcé la fiabilité auprès des marques."},{n:"2.",t:"Performance des vues",d:"Les contenus ont généré de bons volumes de vues, confirmant l'intérêt de l'audience pour les formats proposés."},{n:"3.",t:"Budgets mieux valorisés",d:"Les opérations ont permis de générer un chiffre d'affaires important, notamment grâce à une meilleure capacité de livraison."},{n:"4.",t:"Qualité de production",d:"Le niveau d'exécution est resté élevé malgré plusieurs opérations menées en parallèle."},{n:"5.",t:"Fluidité opérationnelle",d:"La coordination entre Antoine, Nassia, Vincent, et les prestataires a permis de réduire les blocages et d'avancer plus efficacement."}].map((x,i)=><Wc key={i} t={t} s={{padding:"14px 16px",boxShadow:t.cS}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,marginBottom:6}}>{x.n} {x.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.5}}>{x.d}</div></Wc>)}</div></div><div style={{flex:"0.85 1 260px",borderRadius:18,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,minHeight:360,background:t.card}}><img src={bilMai("image15.jpg")} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block",minHeight:360}}/></div></div></div>},
{title:"Points d'amélioration",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",gap:24,flexWrap:"wrap",alignItems:"stretch"}}><div style={{flex:"1.1 1 400px"}}><Tg t={t}>AXES DE PROGRÈS</Tg><Hl t={t} s={{fontSize:28,marginBottom:16}}>POINTS D'AMÉLIORATIONS :</Hl><div style={{display:"flex",flexDirection:"column",gap:14}}>{[{n:"1.",t:"Sélection des marques / affinité avec l'audience",d:"Certaines opérations, notamment avec des marques de jeux mobiles, ont montré un intérêt avec l'univers d'Antoine mais en terme de rétention tenir le viewer devient plus difficile. L'enjeu est de mieux qualifier l'affinité marque / audience / concept en amont."},{n:"2.",t:"Délais de brief / Arknights",d:"Sur certaines campagnes, les briefs ont été reçus trop tardivement, ce qui a réduit le temps disponible pour la préparation créative et la production. L'objectif est de mieux sécuriser les timings en amont."},{n:"3.",t:"Taux d'engagement à contextualiser",d:"Les taux d'engagement sur certaines vidéos ont été plus faibles que prévu, notamment en raison d'une logique de visibilité sur la TV. Il reste un travail de sensibilisation des marques sur les bons KPI à analyser selon les objectifs de campagne."}].map((x,i)=><Wc key={i} t={t} s={{padding:"14px 16px",boxShadow:t.cS}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,marginBottom:6}}>{x.n} {x.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.52}}>{x.d}</div></Wc>)}</div></div><div style={{flex:"0.9 1 280px",borderRadius:18,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,minHeight:380,background:t.card}}><img src={bilMai("image22.jpg")} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block",minHeight:380}}/></div></div></div>},
{title:"Nouveaux process",
r:(t)=><div><FarHeader t={t}/><Tg t={t}>2 — PROCESS</Tg><Hl t={t} s={{fontSize:30,marginBottom:18}}>2- De nouveaux process mis en place</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Wc t={t} s={{padding:20,boxShadow:t.cS}}><Lb t={t}>Côté marques</Lb><div style={{display:"flex",flexDirection:"column",gap:10}}>{["Validation du brief avant signature du contrat","Plus de flexibilité négociée sur les placements dans les vidéos","Clarification systématique des KPI attendus","Meilleure transparence sur les attentes créatives et business","Anticipation des points bloquants avant lancement de campagne"].map((x,i)=><div key={i} style={{...sa,fontSize:14,color:t.m,lineHeight:1.5,paddingLeft:14,borderLeft:`3px solid ${t.a}`}}>{x}</div>)}</div></Wc><Wc t={t} s={{padding:20,boxShadow:t.cS}}><Lb t={t}>Côté AL industries</Lb><div style={{display:"flex",flexDirection:"column",gap:10}}>{["Premier contact systématique avec Nassia pour cadrer les demandes","Double validation des informations par message puis appel","Synthèse des keypoints avant création des scripts","Transmission des éléments clés à Antoine dans un format plus clair","Meilleure anticipation des deadlines et besoins de production"].map((x,i)=><div key={i} style={{...sa,fontSize:14,color:t.m,lineHeight:1.5,paddingLeft:14,borderLeft:`3px solid ${t.a}`}}>{x}</div>)}</div></Wc></div></div>},
{title:"Opportunités non-abouties",
r:(t)=><div><FarHeader t={t}/><Tg t={t}>3 — PIPELINE</Tg><Hl t={t} s={{fontSize:32,marginBottom:16}}>3- Des opportunités non-abouties</Hl><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:10}}>{["image2.jpg","image7.png","image20.png","image24.png","image4.jpg"].map((f)=><div key={f} style={{borderRadius:14,background:t.card,border:`1px solid ${t.brd}`,padding:10,display:"flex",alignItems:"center",justifyContent:"center",minHeight:130,boxShadow:t.cS}}><img src={bilMai(f)} alt="" style={{maxWidth:"100%",maxHeight:150,objectFit:"contain"}}/></div>)}</div></div>},
{title:"Négociations en cours",
r:(t)=><div><FarHeader t={t}/><Tg t={t}>PIPELINE</Tg><Hl t={t} s={{fontSize:30,marginBottom:16}}>LES NEGOCIATIONS EN COURS</Hl><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10}}>{["image6.jpg","image8.png","image14.png","image9.png","image3.png","image13.jpg","image17.jpg"].map((f)=><div key={f} style={{borderRadius:14,background:t.card,border:`1px solid ${t.brd}`,padding:10,display:"flex",alignItems:"center",justifyContent:"center",minHeight:120,boxShadow:t.cS}}><img src={bilMai(f)} alt="" style={{maxWidth:"100%",maxHeight:130,objectFit:"contain"}}/></div>)}</div></div>},
{title:"Recrutement Vincent",
r:(t)=><div><FarHeader t={t}/><Tg t={t}>4 — ÉQUIPE</Tg><Hl t={t} s={{fontSize:28,lineHeight:1.2,marginBottom:12}}>4- Un recrutement qui a transformé l'exécution</Hl><div style={{...se,fontSize:20,fontWeight:800,color:t.a,marginBottom:8}}>Succès du recrutement de Vincent — Chef monteur</div><div style={{...sa,fontSize:16,color:t.m,marginBottom:16,lineHeight:1.55}}>Objectif : sécuriser la livraison des opérations et fluidifier la post-production.</div><div style={{display:"flex",flexDirection:"column",gap:10}}>{["Recrutement d'un profil clé pour reprendre la main sur les montages","Coordination des prestataires externes travaillant sur les contenus d'Antoine","Meilleure fluidité dans la chaîne de production","Capacité renforcée à livrer les opérations dans les délais"].map((x,i)=><div key={i} style={{...sa,fontSize:15,color:t.c,lineHeight:1.5,paddingLeft:14,borderLeft:`3px solid ${t.a}`}}>{x}</div>)}</div><div style={{marginTop:18,padding:"14px 16px",borderRadius:12,background:t.th2,border:`1px solid ${t.brd}`}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,marginBottom:6}}>→ Avril :</div><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.55}}>3 opérations sorties dans les temps, avec un impact direct sur le chiffre d'affaires à court terme</div></div></div>},
{title:"Directeur de production",
r:(t)=><div><FarHeader t={t}/><Tg t={t}>SUITE</Tg><Hl t={t} s={{fontSize:26,lineHeight:1.2,marginBottom:12}}>PROCHAINE ÉTAPE — UN DIRECTEUR DE PRODUCTION ?</Hl><div style={{...se,fontSize:20,fontWeight:800,color:t.c,marginBottom:6}}>Construire une vision long terme.</div><div style={{...sa,fontSize:16,color:t.m,marginBottom:14,lineHeight:1.55}}>Objectif : anticiper davantage, structurer la préproduction</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>{["Accompagner Antoine dans la préproduction des concepts","Structurer le calendrier de production sans brider sa liberté créative","Donner plus de visibilité sur les temps forts à venir","Construire une vision annuelle des projets et activations","Mieux anticiper les besoins humains, techniques et budgétaires"].map((x,i)=><Wc key={i} t={t} s={{padding:"12px 14px",boxShadow:t.cS}}><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.45}}>{x}</div></Wc>)}</div><Wc t={t} s={{marginTop:16,padding:16,boxShadow:t.cS}}><div style={{...sa,fontSize:15,color:t.c,lineHeight:1.55}}><span style={{fontWeight:800,color:t.a}}>→</span> Passer d'une logique <span style={{fontWeight:700}}>d'exécution court terme</span> à une <span style={{fontWeight:700}}>logique de développement long terme</span></div></Wc></div>},
{title:"Autres",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",gap:18,minHeight:"52vh",justifyContent:"center"}}><Tg t={t}>5 — OUVERTURE</Tg><div style={{...se,fontSize:36,fontWeight:900,color:t.c,lineHeight:1.15}}>DES ENVIES ?</div><div style={{...se,fontSize:36,fontWeight:900,color:t.c,lineHeight:1.15}}>DES BESOINS ?</div><div style={{...se,fontSize:36,fontWeight:900,color:t.c,lineHeight:1.15}}>DES PROJETS ?</div><div style={{...mo,fontSize:14,letterSpacing:4,color:t.d}}>5-</div><div style={{maxWidth:520,width:"100%",marginTop:8,borderRadius:16,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,background:t.card}}><img src={bilMai("image25.png")} alt="" style={{width:"100%",display:"block",objectFit:"contain"}}/></div></div></div>},
{title:"Questions",
r:(t)=><div><FarHeader t={t}/><div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"58vh",gap:22,textAlign:"center"}}><div style={{...se,fontSize:44,fontWeight:900,color:t.c}}>DES QUESTIONS ?</div><div style={{maxWidth:480,width:"100%",borderRadius:18,overflow:"hidden",border:`3px solid ${t.brd}`,boxShadow:t.cS,background:t.card}}><img src={bilMai("image19.png")} alt="" style={{width:"100%",display:"block",objectFit:"contain"}}/></div></div></div>},
];
...
// DATA + META
// ═══════════════════════════════════════════════════════════════════════════════
const ALL = { case1:S1, case2:S2, shopify:SS, rode:SR, fastgoodcuisine:SFGC, fgcmarque:SFGCMarque, otacospepe:SOtacosPepe, toinelag:SToinelag, bilantoinelag:SBilanToinelagMai2026, cyrilmp4:SCyril, garmin:SGarmin, profil:SProfil };
/** Liens partagés / SEO informel : id court → id interne (ex. deck « sur la route » = CYRILmp4). */
const DECK_ALIASES = { route: "cyrilmp4", cyril: "cyrilmp4", cyrilmp: "cyrilmp4", garmincyril: "garmin", fgc: "fastgoodcuisine", fastgood: "fastgoodcuisine", fgcx: "fgcmarque", fgcmarque: "fgcmarque", fastgoodmarque: "fgcmarque", otacos: "otacospepe", pepeotacos: "otacospepe", toine: "toinelag", tl: "toinelag", bilanmaitoinelag: "bilantoinelag", bilantoinelagmai2026: "bilantoinelag" };
...
const META = {
  ...
  bilantoinelag:{l:"Bilan mai 2026 · Toinelag",s:"Synthèse mai 2026 — succès, process, pipeline & perspectives",tag:"BILAN",card:"toinelag"},
...
function Home({onOpen}) {
  const cs = {
    ...
    bilantoinelag:{bg:"#FFE14A",c:"#141414",tBg:"rgba(30,116,232,.22)",tC:"#0F3D7A"},
  ...
  return (
    <div className="far-home" style={{minHeight:"100vh",background:A,padding:"44px 52px",...sa,color:B}}>
      ...
      <div className="far-home-grid" style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:20,maxWidth:1155}}>
        {[{id:"shopify",img:"/shopify-logo.png",imgW:110,filter:"brightness(0) invert(1)"},{id:"rode",img:"/rode-logo-white.png",imgW:80,filter:"none"},{id:"fastgoodcuisine",img:"/fgc.webp",imgW:76,filter:"none"},{id:"fgcmarque",img:"/fgc.webp",imgW:76,filter:"none"},{id:"otacospepe",img:"/otacos-logo.png",imgW:118,filter:"none"},{id:"toinelag",img:"/toinelag-avatar.png",imgW:78,filter:"none"},{id:"bilantoinelag",img:"/toinelag-avatar.png",imgW:78,filter:"none"},{id:"cyrilmp4",img:"/cyrilmp4.png",imgW:80,filter:"none",imgType:"photo"},{id:"garmin",img:"/cyrilmp4.png",imgW:80,filter:"none",imgType:"photo"}].map(({id,img,imgW,filter,imgType},idx)=>{const d=META[id];const st=cs[d.card];return(
          <DeckCard key={id} id={id} st={st} d={d} onOpen={onOpen} delay={.5+idx*.1}>
            {imgType==="photo"?<img src={pu(img)} alt="" style={{width:imgW,height:imgW,borderRadius:"50%",objectFit:"cover",border:"3px solid rgba(255,255,255,.2)",flexShrink:0}}/>:<img src={pu(img)} alt="" style={{width:imgW,height:"auto",filter,opacity:.92,flexShrink:0,borderRadius:10,border: id==="toinelag"||id==="bilantoinelag" ? "3px solid rgba(20,20,20,.15)" : id==="fgcmarque" ? "2px solid rgba(28,20,16,.12)" : "none"}}/>}
          </DeckCard>
        );})}
      </div>

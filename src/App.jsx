...
const NetSchema = ({t}) => (
  <div style={{margin:"6px 0 8px"}}>
    <svg viewBox="0 0 900 320" style={{width:"100%",height:"auto",display:"block"}}>
      <defs>
        <marker id="arrStratDown" markerWidth="9" markerHeight="9" refX="4.5" refY="7" orient="auto">
          <path d="M0 0 L9 0 L4.5 7 Z" fill={t.d}/>
        </marker>
        <marker id="arrStratBack" markerWidth="10" markerHeight="10" refX="7" refY="5" orient="auto">
          <path d="M0 0 L9 5 L0 10 Z" fill={t.a}/>
        </marker>
        <clipPath id="netBowlStrat"><path d="M40 195 Q 450 275 860 195 L 860 305 L 40 305 Z"/></clipPath>
      </defs>

      {/* PIPELINE */}
      <rect x="40" y="22" width="820" height="44" rx="22" fill="#1A1A1A"/>
      <text x="60" y="49" fill="#fff" fontSize="13" fontWeight="700" letterSpacing="3" fontFamily="'JetBrains Mono',monospace">PIPELINE COMMERCIAL FAR</text>
      <text x="824" y="49" fill={t.a} fontSize="13" fontWeight="700" letterSpacing="2" textAnchor="end" fontFamily="'JetBrains Mono',monospace">›››</text>

      {/* SECTION LABEL */}
      <text x="40" y="92" fill={t.c} fontSize="10" fontWeight="800" letterSpacing="2.5" fontFamily="'JetBrains Mono',monospace">DEALS QUI MENACENT DE TOMBER</text>
      <line x1="280" y1="88" x2="860" y2="88" stroke={t.brd} strokeWidth="1"/>

      {/* drops with mini labels */}
      {[{cx:130,l:"brief perdu"},{cx:295,l:"reporté"},{cx:450,l:"budget bas"},{cx:605,l:"timing"},{cx:770,l:"marque pas prête"}].map((d,i)=>(
        <g key={i}>
          <text x={d.cx} y={114} fill={t.m} fontSize="11" fontWeight="600" textAnchor="middle" fontFamily="'Figtree',sans-serif">{d.l}</text>
          <line x1={d.cx} y1="124" x2={d.cx} y2="178" stroke={t.d} strokeDasharray="3 4" strokeWidth="1.4" markerEnd="url(#arrStratDown)"/>
        </g>
      ))}

      {/* NET BOWL */}
      <path d="M40 195 Q 450 275 860 195" stroke={t.a} strokeWidth="2.6" fill="none"/>
      <g clipPath="url(#netBowlStrat)" stroke={t.a} strokeWidth="0.7" fill="none" opacity="0.55">
        {Array.from({length:24}).map((_,i)=>{const x=20+i*44;return <line key={`d1${i}`} x1={x} y1="190" x2={x+100} y2="315"/>;})}
        {Array.from({length:24}).map((_,i)=>{const x=20+i*44;return <line key={`d2${i}`} x1={x} y1="190" x2={x-100} y2="315"/>;})}
      </g>

      {/* caught opportunities */}
      {[{cx:430,cy:252},{cx:470,cy:252},{cx:412,cy:260},{cx:450,cy:260},{cx:488,cy:260},{cx:431,cy:244},{cx:469,cy:244}].map((d,i)=>(
        <circle key={`cap${i}`} cx={d.cx} cy={d.cy} r="7" fill={t.a} stroke="#1A1A1A" strokeWidth="1.2"/>
      ))}

      {/* bottom net label */}
      <line x1="40" y1="305" x2="370" y2="305" stroke={t.brd} strokeWidth="1"/>
      <text x="860" y="306" fill={t.c} fontSize="10" fontWeight="800" letterSpacing="2.5" textAnchor="end" fontFamily="'JetBrains Mono',monospace">FILET FAR · TOUT EST CAPTÉ</text>

      {/* feedback arrow + label */}
      <path d="M820 250 C 880 210, 880 110, 820 66" stroke={t.a} strokeWidth="2.2" fill="none" strokeDasharray="6 4" markerEnd="url(#arrStratBack)"/>
      <text x="868" y="160" fill={t.c} fontSize="10" fontWeight="800" letterSpacing="2.5" textAnchor="middle" transform="rotate(-90 868 160)" fontFamily="'JetBrains Mono',monospace">REMIS EN CIRCULATION</text>
    </svg>
  </div>
);
  ...
  return (
    <div style={{margin:"18px 0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
        {branches.map((b,i)=>(
          <div key={i} style={{padding:"14px 14px 16px",borderRadius:12,background:t.card,border:`1px solid ${t.brd}`,borderTop:`3px solid ${t.a}`,display:"flex",flexDirection:"column"}}>
            <div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:2,marginBottom:6}}>{b.k}</div>
            <div style={{...sa,fontSize:13,fontWeight:800,color:t.cardT,marginBottom:10,lineHeight:1.25}}>{b.t}</div>
...
r:t=><div><Tg t={t}>OBJECTIF</Tg><Hl t={t} s={{fontSize:40,marginBottom:12}}>Un grand filet tendu tout au long du pipeline.</Hl><div style={{...sa,fontSize:15.5,color:t.m,lineHeight:1.55,maxWidth:900,marginBottom:4}}>Un deal peut tomber pour mille raisons. L'objectif est que rien ne tombe vraiment dans le vide : chaque interaction est captée, suivie, relancée, enrichie.</div><NetSchema t={t}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:6}}><div style={{padding:"18px 22px",borderRadius:14,background:t.card,border:`1px solid ${t.brd}`}}><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:8}}><div style={{...mo,fontSize:10,fontWeight:800,color:t.a,letterSpacing:2.5}}>BRIEF NON ABOUTI</div><div style={{...mo,fontSize:10,fontWeight:600,color:t.d,letterSpacing:1.5}}>· LE FILET RETIENT</div></div><div style={{...sa,fontSize:13.5,color:t.cardT,lineHeight:1.7}}>contact · besoin · budget potentiel · calendrier · objections · prochains temps forts</div></div><div style={{padding:"18px 22px",borderRadius:14,background:"#1A1A1A",border:"1px solid #2A2A2A"}}><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:8}}><div style={{...mo,fontSize:10,fontWeight:800,color:t.a,letterSpacing:2.5}}>DEAL SIGNÉ</div><div style={{...mo,fontSize:10,fontWeight:600,color:"#888",letterSpacing:1.5}}>· LE FILET OUVRE</div></div><div style={{...sa,fontSize:13.5,color:"#E8E8E8",lineHeight:1.7}}>une suite avec la marque · un format plus ambitieux · une relation plus directe · un ambassadorat · un co-branding</div></div></div><div style={{marginTop:18,padding:"14px 22px",borderLeft:`3px solid ${t.a}`}}><div style={{...se,fontSize:20,fontWeight:800,color:t.c,lineHeight:1.4,fontStyle:"italic"}}>Un brief reçu, même perdu, doit enrichir FAR. Un deal signé, même ponctuel, doit ouvrir une suite.</div></div></div>},

...
// ═══════════════════════════════════════════════════════════════════════════════
// FAR · POSITIONNEMENT  -  15 SLIDES  -  même DA que Cas pratique 1 (T1)
// ═══════════════════════════════════════════════════════════════════════════════
const SFarPositionnement = [
{title:"Ouverture",
r:t=><div style={{textAlign:"center",padding:"36px 0 24px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{marginBottom:36}}><FarLogo size={110} variant={t.lv}/></div><Tg t={t}>POSITIONNEMENT FAR</Tg><Hl t={t} s={{fontSize:46,maxWidth:980,margin:"0 auto 20px",textAlign:"center"}}>FAR donne des perspectives aux collaborations créateur-marque.</Hl><div style={{...se,fontSize:28,fontWeight:800,color:t.a,marginBottom:16}}>FAR, l'agence qui donne des perspectives.</div><Sh t={t} s={{maxWidth:820,margin:"0 auto 28px"}}>FAR aide les marques et les créateurs à construire des collaborations fluides, authentiques, sur-mesure et fidèles à l'univers du créateur.</Sh><div style={{padding:"28px 40px",borderRadius:16,background:"#1A1A1A",color:"#FFFFFF",maxWidth:900,margin:"0 auto 20px"}}><div style={{...se,fontSize:34,fontWeight:800,lineHeight:1.15}}>Un projet qui ouvre une suite.</div></div><div style={{...mo,fontSize:13,fontWeight:600,letterSpacing:4,color:t.d}}>Qui veut aller loin, vient ici.</div></div>},

{title:"Posture, méthode, preuves",
r:t=><div><Tg t={t}>PLATEFORME</Tg><Hl t={t} s={{fontSize:40}}>Trois choses en même temps.</Hl><Sh t={t} white>Une posture, une méthode et des preuves : le positionnement FAR se lit comme une plateforme commerciale claire.</Sh><G2 s={{marginTop:16}}><Wc t={t} s={{padding:28}}><Lb t={t}>POSTURE</Lb><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.65}}>Le mot central côté marques et talents : <span style={{fontWeight:700,color:t.cardT}}>perspectives</span>. Une collaboration n'est pas une prise de parole en plus : c'est un projet qui ouvre une suite.</div></Wc><Wc t={t} s={{padding:28}}><Lb t={t}>MÉTHODE</Lb><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.65}}>Cinq piliers déjà portés par la marque — Vision, Sérénité, Structure, Ambition, Opportunité — deviennent les critères d'un bon projet FAR.</div></Wc></G2><div style={{marginTop:16}}><Wc t={t} s={{padding:28}}><Lb t={t}>PREUVES</Lb><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.65}}>Des cas publics qui parlent déjà le bon langage : fidèle, fluide, authentique, co-construit. Et un roster d'univers activables.</div></Wc></div></div>},

{title:"Le marché s'est structuré",
r:t=><div><Tg t={t}>CONTEXTE</Tg><Hl t={t} s={{fontSize:40}}>L'influence est structurée ; la collaboration, souvent encore sous-construite.</Hl><Sh t={t} white>Le marché ne manque plus de créateurs, d'outils, de plateformes ni de campagnes. Plus le marché se structure, plus l'exigence monte.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:20}}><Wc t={t} s={{padding:24}}><Lb t={t}>SIGNAUX MARCHÉ</Lb><Ar t={t} sm items={["3 Français sur 4 déclarent avoir déjà acheté un produit après un partenariat créateur","Plus de 15 000 campagnes de gifting, 321 000 publications, 1,4 milliard de vues et 80 millions d'interactions sur une fenêtre récente (marché piloté par la donnée et la performance)"]}/></Wc><Wc t={t} s={{padding:24,background:"#1A1A1A",border:"1px solid #333"}}><Lb t={t} w>LA RUPTURE</Lb><div style={{...se,fontSize:26,fontWeight:800,color:"#FFFFFF",marginBottom:10}}>Accès ≠ justesse</div><div style={{...sa,fontSize:14,color:"#CCCCCC",lineHeight:1.6}}>Le problème n'est plus l'accès. Le problème, c'est la justesse : la marque doit être légitime, le créateur doit rester cohérent, l'audience doit sentir qu'il y a une vraie raison d'être.</div></Wc></div></div>},

{title:"Le paysage concurrentiel",
r:t=><div><Tg t={t}>BENCHMARK</Tg><Hl t={t} s={{fontSize:36}}>Cinq routes déjà occupées sur le marché.</Hl><Sh t={t} white>FAR ne doit pas promettre d'être plus gros, plus international ou plus « career management ». FAR doit être plus précis.</Sh><div style={{display:"flex",flexDirection:"column",gap:0,marginTop:24,borderRadius:14,overflow:"hidden",border:`1px solid ${t.brd}`}}>{[{r:"Entertainment & star-system",o:"Créateurs installés, événements, formats longs, gaming, campagnes créatives",f:"FAR évite la surenchère volume / spectacle"},{r:"Content, lifestyle & management",o:"Stratégie, production, management de talents",f:"FAR n'est pas une simple boutique de contenus"},{r:"Réseau international, data & média",o:"Accès international, influence, production, média, data",f:"FAR ne vend pas l'échelle comme promesse centrale"},{r:"Plateforme & volume",o:"Profils du nano au macro, campagne experte en quelques clics",f:"FAR ne vend pas le catalogue comme produit principal"},{r:"Accompagnement carrière créateur",o:"Structuration, image, ligne éditoriale, pérennisation",f:"FAR ne remplace pas le créateur dans sa carrière"}].map((row,i)=><div key={i} style={{display:"flex",gap:0,borderBottom:i<4?`1px solid ${t.brd}`:"none",background:i%2===0?t.card:"#FAFAF8"}}><div style={{width:220,padding:"16px 18px",...mo,fontSize:11,fontWeight:700,color:t.a}}>{row.r}</div><div style={{flex:1,padding:"16px 18px",...sa,fontSize:13,color:t.m,lineHeight:1.5}}>{row.o}</div><div style={{width:260,padding:"16px 18px",...sa,fontSize:13,fontWeight:600,color:t.cardT,lineHeight:1.45}}>{row.f}</div></div>)}</div><div style={{marginTop:20,padding:"16px 28px",borderRadius:14,background:t.th,color:t.thT,textAlign:"center"}}><span style={{fontWeight:700}}>FAR</span> se concentre sur la collaboration juste : <span style={{fontStyle:"italic"}}>au bon endroit, dans le bon univers, avec une perspective après la diffusion</span>.</div></div>},

{title:"Le territoire FAR",
r:t=><div><Tg t={t}>POSITIONNEMENT</Tg><Hl t={t} s={{fontSize:38}}>La collaboration à perspectives.</Hl><Sh t={t} white>Une collaboration à perspectives sert trois intérêts en même temps : marque, créateur, audience.</Sh><G2 s={{marginTop:16}}><Wc t={t} s={{padding:24}}><Lb t={t}>POUR LA MARQUE</Lb><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Trouver un rôle crédible dans l'univers du créateur — pas seulement une visibilité.</div></Wc><Wc t={t} s={{padding:24}}><Lb t={t}>POUR LE CRÉATEUR</Lb><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Se développer sereinement et durablement, sans s'éloigner de sa création.</div></Wc></G2><Wc t={t} s={{padding:24,marginTop:14}}><Lb t={t}>POUR L'AUDIENCE</Lb><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Une collaboration fluide, naturelle, fidèle au contenu : la performance naît de la confiance.</div></Wc><Th t={t} alt>FAR conçoit des collaborations sur-mesure entre marques et créateurs, pensées pour être justes dans le contenu, utiles pour la marque et durables pour le créateur.</Th></div>},

{title:"La méthode en cinq mots",
r:t=><div><Tg t={t}>MÉTHODE</Tg><Hl t={t} s={{fontSize:36}}>Les cinq piliers FAR, version outil de vente.</Hl><Sh t={t} white>Ce ne sont pas des valeurs décoratives : ce sont les critères d'un bon projet FAR.</Sh><div style={{display:"flex",flexDirection:"column",gap:0,marginTop:22,borderRadius:14,overflow:"hidden",border:`1px solid ${t.brd}`}}>{[{p:"Vision",d:"On ne pense pas seulement la diffusion. On pense ce que la collaboration peut ouvrir."},{p:"Sérénité",d:"Un cadre fluide, clair, sur-mesure entre marque, créateur et production."},{p:"Structure",d:"Brief, validation, droits, calendrier, performance : on clarifie et on simplifie."},{p:"Ambition",d:"On ne réduit pas la collaboration à un post quand elle peut devenir un format, une série, une expérience ou une relation."},{p:"Opportunité",d:"On fait émerger des projets innovants et durables, pas seulement des activations ponctuelles."}].map((x,i)=><div key={i} style={{display:"flex",gap:0,borderBottom:i<4?`1px solid ${t.brd}`:"none",background:i%2===0?t.card:"#FAFAF8"}}><div style={{width:160,padding:"18px 20px",...mo,fontSize:13,fontWeight:800,color:t.a}}>{x.p}</div><div style={{flex:1,padding:"18px 20px",...sa,fontSize:14,color:t.m,lineHeight:1.55}}>{x.d}</div></div>)}</div></div>},

{title:"Pour les marques",
r:t=><div><Tg t={t}>MARQUES</Tg><Hl t={t} s={{fontSize:38}}>FAR ne vend pas un créateur : FAR construit un rôle.</Hl><G2 s={{marginTop:20}}><Wc t={t} s={{padding:24,opacity:.85}}><Lb t={t}>LA MAUVAISE QUESTION</Lb><div style={{...se,fontSize:22,fontWeight:800,color:t.cardT,marginBottom:12}}>« Quel créateur peut parler de ma marque ? »</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Une question qui pousse au placement et à la démonstration.</div></Wc><Wc t={t} s={{padding:24,background:t.a+"18",border:`1px solid ${t.a}35`}}><Lb t={t}>LA BONNE QUESTION</Lb><div style={{...se,fontSize:22,fontWeight:800,color:t.cardT,marginBottom:12}}>« Quel rôle ma marque peut jouer dans son univers ? »</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Une question qui pousse à l'intégration fidèle, au ton, au format et à la communauté.</div></Wc></G2><div style={{marginTop:18}}><Ar t={t} items={["Entrer dans un univers créateur déjà installé","Trouver une intégration fidèle au ton, au format et à la communauté","Transformer un message en idée éditoriale","Choisir le bon niveau d'ambition : intégration, ambassade, format, retail, expérience, co-branding","Construire une collaboration défendable, pas seulement visible"]}/></div><Th t={t}>FAR vous aide à devenir utile dans l'univers du créateur.</Th></div>},

{title:"Pour les créateurs",
r:t=><div><Tg t={t}>CRÉATEURS</Tg><Hl t={t} s={{fontSize:38}}>FAR ne vend pas l'audience : FAR protège et développe le terrain de jeu.</Hl><G2 s={{marginTop:20}}><Wc t={t} s={{padding:24,opacity:.85}}><Lb t={t}>LA MAUVAISE PROMESSE</Lb><div style={{...se,fontSize:22,fontWeight:800,color:t.cardT,marginBottom:12}}>« On va monétiser ton audience. »</div></Wc><Wc t={t} s={{padding:24,background:t.a+"18",border:`1px solid ${t.a}35`}}><Lb t={t}>LA BONNE PROMESSE</Lb><div style={{...se,fontSize:22,fontWeight:800,color:t.cardT,marginBottom:12}}>« On t'aide à aller plus loin sans t'éloigner de ta création. »</div></Wc></G2><div style={{marginTop:18}}><Ar t={t} items={["Choisir les collaborations qui renforcent ton univers","Clarifier positionnement, valeurs et ambitions","Structurer revenus, opportunités, process et validations","Gagner en productivité grâce aux outils et à la Suite FAR","Optimiser les performances par la data","Développer formats, concepts, produits ou projets dérivés","Rester concentré sur ce qui t'anime : la création"]}/></div><Th t={t}>FAR t'aide à te développer sereinement et durablement.</Th></div>},

{title:"Six familles de collaborations",
r:t=><div><Tg t={t}>OFFRE</Tg><Hl t={t} s={{fontSize:34}}>Six familles de collaborations à perspectives.</Hl><Sh t={t} white>Chaque famille ouvre une perspective au-delà de la simple diffusion.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:18}}>{[{t:"Intégration fidèle",d:"La marque apparaît dans le contenu sans le dénaturer."},{t:"Ambassade",d:"La collaboration s'installe dans le temps et devient une relation visible."},{t:"Format créateur-marque",d:"Le partenariat devient un concept éditorial."},{t:"Co-branding / produit / licence",d:"Une offre commune entre créateur et marque."},{t:"Retail & drive-to-store",d:"Le contenu impacte le parcours marchand."},{t:"Worlds & experiences",d:"L'univers devient map, live, événement ou hub communautaire."}].map((x,i)=><Wc key={i} t={t} s={{padding:16,boxShadow:"none"}}><div style={{...mo,fontSize:10,fontWeight:800,letterSpacing:1.5,color:t.a,marginBottom:8}}>{String(i+1).padStart(2,"0")}</div><div style={{...sa,fontSize:15,fontWeight:700,color:t.cardT,marginBottom:6}}>{x.t}</div><div style={{...sa,fontSize:12,color:t.m,lineHeight:1.45}}>{x.d}</div></Wc>)}</div></div>},

{title:"Quatre tests qualité",
r:t=><div><Tg t={t}>RÈGLES DU JEU</Tg><Hl t={t} s={{fontSize:38}}>Une collaboration FAR doit passer quatre tests.</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:22}}>{[{k:"Fidélité",q:"Est-ce fidèle à l'univers du créateur ?"},{k:"Fluidité",q:"La marque s'intègre-t-elle naturellement au contenu ?"},{k:"Utilité",q:"La marque apporte-t-elle quelque chose (produit, expérience, idée, accès, expertise, contrainte créative) ?"},{k:"Suite",q:"Le projet peut-il vivre après la diffusion (série, ambassade, retail, co-branding, format, réutilisation) ?"}].map((x,i)=><Wc key={i} t={t} s={{padding:20}}><div style={{...mo,fontSize:12,fontWeight:800,color:t.a,marginBottom:8}}>{x.k}</div><div style={{...sa,fontSize:14,color:t.cardT,lineHeight:1.5}}>{x.q}</div></Wc>)}</div><div style={{marginTop:20,padding:"18px 28px",borderRadius:14,background:"#1A1A1A",color:"#FFFFFF",textAlign:"center"}}><div style={{...sa,fontSize:15,lineHeight:1.55}}>Si la réponse est non sur deux critères, ce n'est pas un projet FAR. <span style={{fontWeight:700}}>Dire non protège l'audience. Protéger l'audience protège la performance.</span></div></div></div>},

{title:"Les univers FAR activables",
r:t=><div><Tg t={t}>ROSTER</Tg><Hl t={t} s={{fontSize:36}}>Des univers, pas un inventaire.</Hl><Sh t={t} white>Les créateurs FAR ne sont pas une liste de profils : ce sont des territoires éditoriaux activables pour des marques.</Sh><div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",justifyContent:"center"}}>{[{v:"10 ans",l:"d'expérience"},{v:"+50",l:"marques"},{v:"+100",l:"campagnes"},{v:"4",l:"créateurs mis en avant"}].map((s,i)=><div key={i} style={{padding:"12px 20px",borderRadius:10,background:t.card,border:`1px solid ${t.brd}`,textAlign:"center",minWidth:120}}><div style={{...se,fontSize:22,fontWeight:800,color:t.a}}>{s.v}</div><div style={{...sa,fontSize:11,color:t.m}}>{s.l}</div></div>)}</div><div style={{display:"flex",flexDirection:"column",gap:0,marginTop:8,borderRadius:14,overflow:"hidden",border:`1px solid ${t.brd}`}}>{[{n:"Fast Good Cuisine",u:"Food entertainment",p:"Tests, défis, formats cuisine, co-brandings, opérations food"},{n:"Toinelag",u:"Construction, jeux, divertissement",p:"Concepts co-construits, jouets, retail, drive-to-store"},{n:"Le Routin",u:"Fast-food, snacks, grande consommation",p:"Recettes, dégustations, formats courts et longs, contenus premium"},{n:"CYRILmp4",u:"Exploration, gaming, divertissement",p:"Ambassades, formats longs, aventures, gaming, projets annuels"}].map((c,i)=><div key={i} style={{display:"flex",gap:0,borderBottom:i<3?`1px solid ${t.brd}`:"none",background:i%2===0?t.card:"#FAFAF8"}}><div style={{width:200,padding:"14px 18px",...sa,fontSize:14,fontWeight:800,color:t.cardT}}>{c.n}</div><div style={{width:220,padding:"14px 18px",...sa,fontSize:13,color:t.m}}>{c.u}</div><div style={{flex:1,padding:"14px 18px",...sa,fontSize:13,color:t.m,lineHeight:1.45}}>{c.p}</div></div>)}</div></div>},

{title:"Preuves publiques",
r:t=><div><Tg t={t}>PREUVES</Tg><Hl t={t} s={{fontSize:36}}>Le bon langage est déjà dans les cas.</Hl><Sh t={t} white>FAR n'a pas besoin d'inventer un nouveau discours : il faut mieux organiser ce que FAR fait déjà.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:18}}>{[{t:"Google Pixel × Caroline Ambrosini",d:"Scénario réfléchi et fidèle à l'univers, présentation subtile du produit, collaboration fluide et authentique."},{t:"Holy × CYRILmp4",d:"Ambassade sur six mois, activée sur la chaîne gaming et la chaîne principale."},{t:"Duolingo × Valise et Caramel",d:"Format fun et immersif, intégré naturellement au contenu."},{t:"Adopte × CYRILmp4",d:"Vidéo co-construite avec la marque, pensée en trois temps : avant, pendant et après la vidéo."}].map((x,i)=><Wc key={i} t={t} s={{padding:20}}><div style={{...sa,fontSize:15,fontWeight:800,color:t.cardT,marginBottom:8}}>{x.t}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.5}}>{x.d}</div></Wc>)}</div><div style={{marginTop:22}}><Lb t={t}>PERSPECTIVES CONCRÈTES</Lb><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.5,marginBottom:10}}>Une collaboration peut devenir un format, un produit, une expérience, un hub, une relation ou une nouvelle source de revenus.</div><div style={{display:"flex",flexDirection:"column",gap:0,borderRadius:12,overflow:"hidden",border:`1px solid ${t.brd}`}}>{[{n:"FastGoodCuisine",m:"Collaborations sur mesure, co-branding, nouveaux formats cuisine.",l:"La food peut devenir format, produit, opération ou co-branding."},{n:"CYRILmp4",m:"Partenariats annuels, développement des chaînes, accélération de formats et d'écosystèmes.",l:"Le créateur peut porter un écosystème de marques et de formats."},{n:"Toinelag",m:"Concepts co-construits, formats drive-to-store.",l:"Le divertissement peut générer une mécanique retail."},{n:"Le Routin",m:"Recettes, formats courts et longs, production premium.",l:"Le contenu peut servir le créateur et la marque."},{n:"Teeqzy",m:"Map, hub d'activation, trafic qualifié.",l:"L'univers gaming peut devenir un monde activable."},{n:"Bek1ng",m:"Coaching, performance, contenus techniques, marque blanche ou co-branding.",l:"L'expertise peut devenir un format à valeur ajoutée."}].map((row,i)=><div key={i} style={{display:"flex",gap:0,borderBottom:i<5?`1px solid ${t.brd}`:"none",background:i%2===0?t.card:"#FAFAF8"}}><div style={{width:130,padding:"10px 14px",...sa,fontSize:12,fontWeight:800,color:t.cardT}}>{row.n}</div><div style={{flex:1,padding:"10px 14px",...sa,fontSize:11,color:t.m,lineHeight:1.4}}>{row.m}</div><div style={{width:200,padding:"10px 14px",...sa,fontSize:11,color:t.m,lineHeight:1.4,fontWeight:600}}>{row.l}</div></div>)}</div></div></div>},

{title:"Ce que FAR ne vend pas",
r:t=><div><Tg t={t}>POSTURE RADICALE</Tg><Hl t={t} s={{fontSize:34}}>Clarifier ce que FAR assume — et ce qu'elle ne vend pas.</Hl><Cmp t={t} l="FAR ne vend pas" lI={["Un catalogue de profils","Une campagne en quelques clics","Du volume pour le volume","Un placement qui interrompt le contenu","Une carrière gérée à la place du créateur","Une production déconnectée de l'audience","Une activation sans suite possible"]} r="FAR construit" rI={["Une collaboration juste","Un rôle de marque clair","Une idée fidèle à l'univers","Un cadre fluide et structuré","Une opportunité durable pour le créateur","Une visibilité qui a du sens"]}/></div>},

{title:"Construire une collaboration en six mouvements",
r:t=><div><Tg t={t}>PROCESS</Tg><Hl t={t} s={{fontSize:32}}>Six mouvements, pas une accumulation de livrables.</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:18}}>{[{n:"01",t:"Lire l'univers",d:"Formats, codes, audience, limites, ambitions."},{n:"02",t:"Qualifier l'opportunité",d:"Pourquoi cette marque, pourquoi maintenant, pourquoi ce créateur."},{n:"03",t:"Définir le rôle de la marque",d:"Ce qu'elle apporte vraiment au contenu et à la communauté."},{n:"04",t:"Designer la collaboration",d:"Concept, mécanique, formats, droits, calendrier, KPIs."},{n:"05",t:"Activer avec structure",d:"Production, coordination, diffusion, conformité, reporting."},{n:"06",t:"Ouvrir la suite",d:"Ambassade, série, retail, co-branding, réutilisation, apprentissage, nouveau format."}].map((s,i)=><Wc key={i} t={t} s={{padding:14,boxShadow:"none"}}><div style={{...mo,fontSize:11,fontWeight:700,color:t.a}}>{s.n}</div><div style={{...sa,fontSize:14,fontWeight:700,color:t.cardT,marginBottom:6}}>{s.t}</div><div style={{...sa,fontSize:12,color:t.m,lineHeight:1.45}}>{s.d}</div></Wc>)}</div><Th t={t} alt>Livrable recommandé : une <span style={{fontWeight:700}}>Perspective Sheet FAR</span> — créateur, univers, rôle de la marque, idée, formats, preuves, KPIs, suite possible.</Th></div>},
...
const NetSchema = ({t}) => (
  <div style={{margin:"4px 0 10px"}}>
    <svg viewBox="0 0 900 280" style={{width:"100%",height:"auto",display:"block"}}>
      <defs>
        <marker id="arrStratDown" markerWidth="8" markerHeight="8" refX="4" refY="6" orient="auto">
          <path d="M0 0 L8 0 L4 6 Z" fill={t.d}/>
        </marker>
        <marker id="arrStratBack" markerWidth="10" markerHeight="10" refX="7" refY="5" orient="auto">
          <path d="M0 0 L9 5 L0 10 Z" fill={t.a}/>
        </marker>
        <clipPath id="netBowlStrat"><path d="M40 145 Q 450 245 860 145 L 860 260 L 40 260 Z"/></clipPath>
      </defs>
      <rect x="40" y="24" width="820" height="44" rx="22" fill="#1A1A1A"/>
      <text x="60" y="51" fill="#fff" fontSize="13" fontWeight="700" letterSpacing="3" style={{...mo}}>PIPELINE COMMERCIAL FAR</text>
      <polygon points="838,28 858,46 838,64" fill={t.a}/>
      {[140,290,450,610,740].map((cx,i)=>(
        <line key={`drop${i}`} x1={cx} y1="68" x2={cx} y2={i===2?138:i===1||i===3?132:126} stroke={t.d} strokeDasharray="3 4" strokeWidth="1.3" markerEnd="url(#arrStratDown)"/>
      ))}
      <path d="M40 145 Q 450 245 860 145" stroke={t.a} strokeWidth="2.6" fill="none"/>
      <g clipPath="url(#netBowlStrat)" stroke={t.a} strokeWidth="0.7" fill="none">
        {Array.from({length:22}).map((_,i)=>{const x=20+i*48;return <line key={`d1${i}`} x1={x} y1="140" x2={x+110} y2="270" opacity="0.55"/>;})}
        {Array.from({length:22}).map((_,i)=>{const x=20+i*48;return <line key={`d2${i}`} x1={x} y1="140" x2={x-110} y2="270" opacity="0.55"/>;})}
      </g>
      {[{cx:430,cy:225},{cx:470,cy:225},{cx:412,cy:233},{cx:450,cy:233},{cx:488,cy:233},{cx:431,cy:217},{cx:469,cy:217}].map((d,i)=>(
        <circle key={`cap${i}`} cx={d.cx} cy={d.cy} r="6.5" fill={t.a} stroke="#1A1A1A" strokeWidth="1"/>
      ))}
      <path d="M820 215 C 880 178, 880 108, 820 68" stroke={t.a} strokeWidth="2.2" fill="none" strokeDasharray="6 4" markerEnd="url(#arrStratBack)"/>
    </svg>
  </div>
);
  ...
  return (
    <div style={{margin:"18px 0"}}>
      <div style={{textAlign:"center",marginBottom:6}}>
        <div style={{display:"inline-block",padding:"11px 24px",borderRadius:999,background:"#1A1A1A",color:"#fff",...mo,fontSize:12,fontWeight:700,letterSpacing:2.5,textTransform:"uppercase"}}>Entrée · conversation marque</div>
      </div>
      <div style={{display:"flex",justifyContent:"center",margin:"0 auto"}}>
        <svg viewBox="0 0 800 36" preserveAspectRatio="none" style={{width:"100%",maxWidth:960,height:24,display:"block"}}>
          <path d="M400 0 L400 14 L80 14 L80 30" stroke={t.a} strokeWidth="1.4" fill="none" opacity="0.7"/>
          <path d="M400 14 L240 14 L240 30" stroke={t.a} strokeWidth="1.4" fill="none" opacity="0.7"/>
          <path d="M400 14 L400 30" stroke={t.a} strokeWidth="1.4" fill="none" opacity="0.7"/>
          <path d="M400 14 L560 14 L560 30" stroke={t.a} strokeWidth="1.4" fill="none" opacity="0.7"/>
          <path d="M400 14 L720 14 L720 30" stroke={t.a} strokeWidth="1.4" fill="none" opacity="0.7"/>
        </svg>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14}}>
        {branches.map((b,i)=>(
          <div key={i} style={{padding:"14px 4px 0",borderTop:`2px solid ${t.a}`,display:"flex",flexDirection:"column"}}>
            <div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:2,marginBottom:6}}>{b.k}</div>
            <div style={{...sa,fontSize:13,fontWeight:800,color:t.c,marginBottom:10,lineHeight:1.25}}>{b.t}</div>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {b.items.map((it,j)=>(
                <div key={j} style={{...sa,fontSize:11.5,color:t.m,lineHeight:1.4}}>{it}</div>
              ))}
            </div>
          </div>
        ))}
...
const PipelineSchema = ({t,steps}) => (
  <div style={{display:"grid",gridTemplateColumns:`repeat(${steps.length},1fr)`,gap:0,margin:"26px 0 28px",borderTop:`2px solid ${t.a}`,paddingTop:14,position:"relative"}}>
    {steps.map((s,i)=>{
      const isClose = i===6;
      return (
        <div key={i} style={{padding:"6px 8px 6px 0",display:"flex",flexDirection:"column",alignItems:"flex-start",borderRight:i<steps.length-1?`1px solid ${t.brd}`:"none",paddingLeft:i===0?0:12,position:"relative"}}>
          {isClose&&<div style={{position:"absolute",top:-16,left:0,right:0,height:6,background:t.a,borderRadius:3}}/>}
          <div style={{...mo,fontSize:10,fontWeight:800,color:isClose?t.a:t.d,letterSpacing:1.5,marginBottom:6}}>{String(i+1).padStart(2,"0")}</div>
          <div style={{...sa,fontSize:11.5,fontWeight:isClose?800:600,lineHeight:1.3,color:isClose?t.c:t.c}}>{s}</div>
        </div>
      );
    })}
  </div>
...
r:t=><div><Tg t={t}>OBJECTIF</Tg><Hl t={t} s={{fontSize:40,marginBottom:14}}>Un grand filet tendu tout au long du pipeline.</Hl><div style={{...sa,fontSize:16,color:t.m,lineHeight:1.55,maxWidth:880,marginBottom:8}}>Un deal peut tomber pour mille raisons. L'objectif est que rien ne tombe vraiment dans le vide : chaque interaction est captée, suivie, relancée, enrichie.</div><NetSchema t={t}/><div style={{display:"grid",gridTemplateColumns:"1fr 1px 1fr",gap:24,marginTop:6}}><div><div style={{...mo,fontSize:10,fontWeight:800,color:t.a,letterSpacing:2.5,marginBottom:10}}>BRIEF NON ABOUTI · LE FILET RETIENT</div><div style={{...sa,fontSize:14,color:t.c,lineHeight:1.85}}>contact · besoin · budget potentiel · calendrier · objections · prochains temps forts</div></div><div style={{background:t.brd}}/><div><div style={{...mo,fontSize:10,fontWeight:800,color:t.a,letterSpacing:2.5,marginBottom:10}}>DEAL SIGNÉ · LE FILET OUVRE</div><div style={{...sa,fontSize:14,color:t.c,lineHeight:1.85}}>une suite avec la marque · un format plus ambitieux · une relation plus directe · un ambassadorat · un co-branding</div></div></div><div style={{marginTop:26,paddingTop:18,borderTop:`1px solid ${t.brd}`,display:"flex",gap:18,alignItems:"baseline"}}><div style={{width:36,height:2,background:t.a,flexShrink:0,position:"relative",top:-6}}/><div style={{...se,fontSize:22,fontWeight:800,color:t.c,lineHeight:1.35,fontStyle:"italic"}}>Un brief reçu, même perdu, doit enrichir FAR. Un deal signé, même ponctuel, doit ouvrir une suite.</div></div></div>},

{title:"Les 3 layers commerciaux",
r:t=><div><Tg t={t}>STRUCTURE</Tg><Hl t={t} s={{fontSize:40,marginBottom:8}}>3 layers, complémentaires.</Hl><div style={{...sa,fontSize:16,color:t.m,lineHeight:1.55,maxWidth:760,marginBottom:24}}>FAR structure son développement autour de 3 niveaux qui se nourrissent les uns les autres.</div><div style={{display:"grid",gridTemplateColumns:"0.85fr 1.15fr",gap:36,alignItems:"center"}}><TargetSchema t={t}/><div style={{display:"flex",flexDirection:"column"}}>{[{k:"01",l:"LAYER 1",t:"Entrant incrémental",d:"Les marques ou agences qui sollicitent déjà FAR pour l'un des 4 talents. Le brief devient une porte d'entrée pour comprendre la marque, enrichir la collaboration et ouvrir une suite."},{k:"02",l:"LAYER 2",t:"Agences influence, pub, média",d:"Les agences qui recommandent, sourcent, arbitrent et gèrent les budgets. Exister auprès des bonnes personnes avant que les briefs ne sortent."},{k:"03",l:"LAYER 3",t:"Marques directes",d:"On ouvre une discussion sans arriver avec une idée figée. Comprendre leur moment, puis ouvrir la bonne branche : ambassadorat, co-branding, activation ou contenu."}].map((x,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"54px 1fr",gap:14,padding:"18px 4px 18px 0",borderTop:i===0?`1px solid ${t.brd}`:"none",borderBottom:`1px solid ${t.brd}`,position:"relative"}}>{i===0&&<div style={{position:"absolute",left:-12,top:18,bottom:18,width:3,background:t.a,borderRadius:2}}/>}<div><div style={{...se,fontSize:34,fontWeight:900,color:t.a,lineHeight:1}}>{x.k}</div></div><div><div style={{...mo,fontSize:10,fontWeight:800,color:t.d,letterSpacing:2.5,marginBottom:4}}>{x.l}</div><div style={{...sa,fontSize:17,fontWeight:800,color:t.c,marginBottom:6}}>{x.t}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.55}}>{x.d}</div></div></div>)}</div></div></div>},

{title:"Layer 1 · porte d'entrée",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>LAYER 1 · ENTRANT INCRÉMENTAL</div><Hl t={t} s={{fontSize:42,marginBottom:14}}>Chaque brief est une porte d'entrée.</Hl><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6,maxWidth:900,marginBottom:26}}>Le flux le plus chaud. À traiter vite, jamais comme une simple demande de disponibilité. Avec 4 talents, on n'essaie pas de « placer quelqu'un d'autre » : on exploite mieux l'opportunité autour du talent demandé et on construit la relation.</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0,position:"relative"}}><div style={{position:"absolute",left:"50%",top:14,bottom:8,width:1,background:t.brd}}/><div style={{paddingRight:36}}><div style={{display:"flex",alignItems:"baseline",gap:14,marginBottom:14}}><div style={{...se,fontSize:54,fontWeight:900,color:t.a,lineHeight:1}}>01</div><div style={{...sa,fontSize:18,fontWeight:800,color:t.c}}>Qualifier immédiatement</div></div>{["Pourquoi ce talent précisément ?","Quel est le vrai objectif de la campagne ?","Quel budget — réel ou fourchette ?","One-shot ou temps fort plus large ?","Qui décide ?","Prochains temps forts de la marque ?","Prise de parole ou relation plus incarnée ?"].map((q,i)=><div key={i} style={{...sa,fontSize:13.5,color:t.c,padding:"8px 0",borderBottom:i<6?`1px solid ${t.brd}`:"none",lineHeight:1.5}}>{q}</div>)}</div><div style={{paddingLeft:36}}><div style={{display:"flex",alignItems:"baseline",gap:14,marginBottom:14}}><div style={{...se,fontSize:54,fontWeight:900,color:t.a,lineHeight:1}}>02</div><div style={{...sa,fontSize:18,fontWeight:800,color:t.c}}>Répondre avec une lecture</div></div><div style={{...sa,fontSize:13,color:t.m,fontStyle:"italic",marginBottom:10,lineHeight:1.55}}>Pas « oui / non / voici le tarif ». La réponse intègre :</div>{["Une lecture du brief","Le bon angle pour le talent","Les formats qui peuvent fonctionner","Les limites à ne pas dépasser","Les conditions pour que la collaboration soit crédible","Une ouverture sur la suite"].map((q,i)=><div key={i} style={{...sa,fontSize:13.5,color:t.c,padding:"8px 0",borderBottom:i<5?`1px solid ${t.brd}`:"none",lineHeight:1.5,paddingLeft:14,position:"relative"}}><span style={{position:"absolute",left:0,color:t.a,fontWeight:700}}>→</span>{q}</div>)}</div></div></div>},

{title:"Layer 1 · élargir et retenir",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>LAYER 1 · ENTRANT INCRÉMENTAL</div><Hl t={t} s={{fontSize:38,marginBottom:8}}>Même petit, même perdu, un brief produit de la valeur.</Hl><div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr 1fr",gap:0,marginTop:24}}>{[{n:"01",t:"Élargir autour du talent",items:["une vidéo ponctuelle → une série courte","un post social → contenu + relais event","une campagne produit → relation récurrente","une activation simple → présence éditorialisée","un lancement → avant / pendant / après","un one-shot → discussion d'ambassadorat"]},{n:"02",t:"Garder la relation",items:["récupérer le bon contact","comprendre le prochain moment de marque","noter le type de budget","identifier les catégories à venir","savoir quand revenir"]},{n:"03",t:"Post-campagne obligatoire",items:["retour simple sur ce qui a marché","prochains temps forts","autres équipes ou interlocuteurs","suite plus ambitieuse à ouvrir"]}].map((col,i)=><div key={i} style={{padding:"0 20px",borderRight:i<2?`1px solid ${t.brd}`:"none"}}><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:14}}><div style={{...se,fontSize:30,fontWeight:900,color:t.a,lineHeight:1}}>{col.n}</div><div style={{...sa,fontSize:15,fontWeight:800,color:t.c,lineHeight:1.25}}>{col.t}</div></div><div style={{display:"flex",flexDirection:"column",gap:7}}>{col.items.map((it,j)=><div key={j} style={{...sa,fontSize:12.5,color:t.m,lineHeight:1.5}}>{it}</div>)}</div></div>)}</div><div style={{marginTop:30,paddingTop:18,borderTop:`1px solid ${t.brd}`,display:"flex",alignItems:"center",gap:10}}>{["Brief reçu","Qualification","Collaboration ou abandon","Contact retenu","Prochaine opportunité"].map((x,i)=><span key={i} style={{display:"flex",alignItems:"center",gap:10,flex:1}}><span style={{...mo,fontSize:11,fontWeight:700,letterSpacing:1.5,color:i===4?t.a:t.m,flex:1,textAlign:"center"}}>{x.toUpperCase()}</span>{i<4&&<span style={{color:t.d,fontSize:13}}>›</span>}</span>)}</div><div style={{marginTop:22,...se,fontSize:22,fontWeight:800,color:t.c,fontStyle:"italic"}}>Un brief perdu doit devenir une future opportunité qualifiée.</div></div>},

{title:"Layer 2 · devenir un réflexe agences",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>LAYER 2 · AGENCES</div><Hl t={t} s={{fontSize:42,marginBottom:14}}>Devenir un réflexe avant que le brief ne sorte.</Hl><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6,maxWidth:900,marginBottom:30}}>Les agences voient passer plusieurs marques, plusieurs briefs et plusieurs budgets. Avec 4 talents, on n'essaie pas d'être partout — il faut qu'elles comprennent précisément <span style={{color:t.c,fontWeight:700}}>qui sont les talents, dans quels contextes ils sont forts et pourquoi ils sont crédibles</span>.</div><div style={{display:"flex",flexDirection:"column",gap:0}}>{[{n:"01",t:"Mapper les bonnes agences",render:()=><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{["Influence","Social / Content","Pub","Média","Événementiel","Intégrées (grands groupes)"].map((x,i)=><span key={i} style={{...mo,fontSize:11,fontWeight:700,padding:"7px 14px",borderRadius:999,background:"transparent",color:t.c,border:`1px solid ${t.brd}`}}>{x}</span>)}</div>},{n:"02",t:"Dans chaque agence, identifier",render:()=><div style={{...sa,fontSize:13.5,color:t.m,lineHeight:1.85}}>les personnes qui sourcent les talents · celles qui recommandent aux marques · celles qui tiennent la relation client · celles qui gèrent les budgets influence, social et contenu</div>},{n:"03",t:"Créer la relation hors brief",render:()=><div style={{...sa,fontSize:13.5,color:t.m,lineHeight:1.85}}>cafés · visios courtes · déjeuners · updates ciblés · rencontres autour d'un talent ou d'un territoire</div>}].map((row,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"70px 260px 1fr",gap:24,alignItems:"start",padding:"18px 0",borderTop:`1px solid ${t.brd}`,borderBottom:i===2?`1px solid ${t.brd}`:"none"}}><div style={{...se,fontSize:30,fontWeight:900,color:t.a,lineHeight:1}}>{row.n}</div><div style={{...sa,fontSize:16,fontWeight:800,color:t.c,paddingTop:6}}>{row.t}</div><div style={{paddingTop:4}}>{row.render()}</div></div>)}</div></div>},

{title:"Layer 2 · donner de la matière utile",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>LAYER 2 · AGENCES</div><Hl t={t} s={{fontSize:42,marginBottom:30}}>Utile à la réflexion, pas seulement au brief.</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0,position:"relative"}}><div style={{position:"absolute",left:"50%",top:0,bottom:0,width:1,background:t.brd}}/><div style={{paddingRight:38}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:12}}>PAS UN DECK GÉNÉRIQUE</div><div style={{...se,fontSize:24,fontWeight:800,color:t.c,marginBottom:18,lineHeight:1.2}}>Une matière courte et concrète.</div>{["Les 4 talents","Leur territoire","Leurs formats forts","Les catégories de marques pertinentes","Les collaborations à éviter","Les moments d'actualité","Les formats imaginables avec eux"].map((x,i)=><div key={i} style={{...sa,fontSize:14,color:t.c,padding:"9px 0",borderBottom:i<6?`1px solid ${t.brd}`:"none",lineHeight:1.45}}>{x}</div>)}</div><div style={{paddingLeft:38}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:12}}>AIDER L'AGENCE À MIEUX PENSER</div><div style={{...se,fontSize:24,fontWeight:800,color:t.c,marginBottom:18,lineHeight:1.2}}>L'usage d'un talent.</div>{["Le brief est-il crédible ?","Le format est-il naturel ?","La marque est-elle compatible ?","Le talent peut-il incarner le sujet ?","Y a-t-il un risque d'intégration forcée ?"].map((x,i)=><div key={i} style={{...sa,fontSize:14,color:t.c,padding:"9px 0",borderBottom:i<4?`1px solid ${t.brd}`:"none",lineHeight:1.45,paddingLeft:16,position:"relative"}}><span style={{position:"absolute",left:0,color:t.a,fontWeight:700}}>?</span>{x}</div>)}</div></div><div style={{marginTop:28,paddingTop:18,borderTop:`1px solid ${t.brd}`,display:"flex",gap:18,alignItems:"baseline"}}><div style={{width:36,height:2,background:t.a,flexShrink:0,position:"relative",top:-6}}/><div style={{...se,fontSize:20,fontWeight:800,color:t.c,fontStyle:"italic",lineHeight:1.4}}>L'agence voit FAR comme un interlocuteur qui protège la qualité — pas comme un fournisseur de talents.</div></div></div>},
...
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>PRIORITÉS MARQUES</div><Hl t={t} s={{fontSize:42,marginBottom:14}}>Avec 4 talents, on choisit avec précision.</Hl><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6,maxWidth:900,marginBottom:26}}>Pas des logos pour des logos. On va chercher les marques qui peuvent <span style={{color:t.c,fontWeight:700}}>réellement construire quelque chose</span> avec les talents existants.</div><div style={{display:"grid",gridTemplateColumns:"1.15fr 1fr",gap:48}}><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:14}}>CRITÈRES DE PRIORITÉ</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 22px"}}>{["Capacité à travailler sur plusieurs temps","Capacité de budget suffisante","Besoin d'incarnation","Recherche de désirabilité","Au-delà du post sponsorisé","Potentiel d'ambassadorat","Potentiel de co-branding","Temps forts réguliers","Compatibilité avec un univers FAR"].map((x,i)=><div key={i} style={{...sa,fontSize:13,color:t.c,padding:"7px 0",borderBottom:`1px solid ${t.brd}`,lineHeight:1.4}}>{x}</div>)}</div></div><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:14}}>TYPOLOGIES À VISER</div>{["Marques déjà habituées aux talents","Marques premium qui veulent rajeunir leur image","Challenger brands","Marques avec lancements fréquents","Marques qui ont besoin d'incarnation"].map((x,i)=><div key={i} style={{display:"flex",alignItems:"baseline",gap:16,padding:"11px 0",borderTop:i===0?`1px solid ${t.brd}`:"none",borderBottom:`1px solid ${t.brd}`}}><span style={{...se,fontSize:22,fontWeight:900,color:t.a,lineHeight:1,minWidth:38}}>{String(i+1).padStart(2,"0")}</span><span style={{...sa,fontSize:14,color:t.c,fontWeight:600,lineHeight:1.4}}>{x}</span></div>)}</div></div><div style={{marginTop:28,paddingTop:18,borderTop:`1px solid ${t.brd}`}}><div style={{...mo,fontSize:11,fontWeight:800,letterSpacing:2,color:t.a,marginBottom:14}}>FICHE MARQUE SIMPLE</div><div style={{display:"flex",alignItems:"center",gap:0}}>{["Pourquoi cette marque","Quel talent FAR crédible","Type d'ouverture","Contact prioritaire","Timing","Prochaine action"].map((x,i)=><div key={i} style={{flex:1,...sa,fontSize:12,color:t.c,fontWeight:600,padding:"0 14px",borderLeft:i>0?`1px solid ${t.brd}`:"none",lineHeight:1.35}}>{x}</div>)}</div></div></div>},

{title:"Système commercial · pipeline",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>SYSTÈME COMMERCIAL</div><Hl t={t} s={{fontSize:42,marginBottom:8}}>Chaque contact, une opportunité.</Hl><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6,maxWidth:880,marginBottom:6}}>Pas un échange isolé. Un suivi en 10 étapes — et un CRM qui répond à une seule question.</div><PipelineSchema t={t} steps={["Contact identifié","Premier échange lancé","Besoin qualifié","Route choisie","Proposition envoyée","Négociation","Signé","Post-campagne","Prochaine opportunité","Relation maintenue"]}/><div style={{display:"grid",gridTemplateColumns:"1.25fr 1fr",gap:48,marginTop:4}}><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:14}}>CE QU'ON GARDE DANS LE CRM</div><div style={{...sa,fontSize:14,color:t.c,lineHeight:1.85}}>contacts clés · historique des échanges · talent concerné · briefs reçus · budgets connus ou supposés · catégories d'intérêt · objections · prochains temps forts · prochaine action · niveau de chaleur</div></div><div style={{display:"flex",flexDirection:"column",justifyContent:"center",borderLeft:`1px solid ${t.brd}`,paddingLeft:36}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:2,marginBottom:14}}>UNE SEULE QUESTION</div><div style={{...se,fontSize:28,fontWeight:800,color:t.c,lineHeight:1.25,marginBottom:10}}>Qu'est-ce qu'on fait ensuite avec ce contact ?</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.55,fontStyle:"italic"}}>Le CRM sert le suivi, pas le reporting.</div></div></div></div>},

{title:"Talents à recruter · repérage",
r:t=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>TALENTS · REPÉRAGE</div><Hl t={t} s={{fontSize:38,marginBottom:14}}>Pas une course au volume, une logique de complémentarité.</Hl><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6,maxWidth:880,marginBottom:24}}>FAR a 4 talents aujourd'hui. Les prochains recrutements comblent des espaces précis, pas des noms en plus.</div><div style={{display:"grid",gridTemplateColumns:"0.85fr 1.15fr",gap:48,alignItems:"start"}}><div style={{paddingTop:8}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:2,marginBottom:14}}>SIGNAL PRIORITAIRE</div><div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:8}}><div style={{...se,fontSize:80,fontWeight:900,color:t.c,lineHeight:.9,letterSpacing:"-0.04em"}}>300<span style={{color:t.a}}>—</span>500</div><div style={{...se,fontSize:36,fontWeight:800,color:t.c,lineHeight:1}}>k</div></div><div style={{...sa,fontSize:14,color:t.c,fontWeight:600,marginBottom:6}}>vues moyennes YouTube à 30 jours</div><div style={{...sa,fontSize:13,color:t.m,fontStyle:"italic",lineHeight:1.5}}>Plus fiable que le nombre d'abonnés.</div></div><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.d,letterSpacing:2,marginBottom:14}}>CE QU'ON REGARDE VRAIMENT</div>{[{t:"Vues moyennes 30j",d:"Plus fiable que les abonnés."},{t:"Régularité",d:"Mieux qu'un hit isolé."},{t:"Territoire clair",d:"L'univers se dit en une phrase."},{t:"Compatibilité marque",d:"Intégrer sans abîmer le contenu."},{t:"Qualité de communauté",d:"Commentaires, références, habitudes."},{t:"Sous-monétisation",d:"Traction forte, potentiel non structuré."},{t:"Complémentarité",d:"Ce qu'il ouvre que FAR n'a pas encore."}].map((x,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"54px 200px 1fr",gap:14,alignItems:"baseline",padding:"10px 0",borderTop:i===0?`1px solid ${t.brd}`:"none",borderBottom:`1px solid ${t.brd}`}}><span style={{...se,fontSize:22,fontWeight:900,color:t.a,lineHeight:1}}>{String(i+1).padStart(2,"0")}</span><span style={{...sa,fontSize:13.5,fontWeight:700,color:t.c}}>{x.t}</span><span style={{...sa,fontSize:12.5,color:t.m,lineHeight:1.45}}>{x.d}</span></div>)}</div></div></div>},

{title:"Pipeline de prise de contact talents",
r:(t,back)=><div><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,letterSpacing:3,marginBottom:14}}>TALENTS · PRISE DE CONTACT</div><Hl t={t} s={{fontSize:38,marginBottom:24}}>3 étapes en amont, jamais d'approche vague.</Hl><div style={{display:"grid",gridTemplateColumns:"1.1fr 1fr 1fr",gap:0,position:"relative"}}>{[{n:"01",t:"Repérage",render:()=><><div style={{...mo,fontSize:10,fontWeight:800,color:t.d,letterSpacing:1.8,marginBottom:8}}>SOURCES</div><div style={{...sa,fontSize:12.5,color:t.c,lineHeight:1.7,marginBottom:18}}>YouTube · TikTok · Twitch · Instagram · recos talents proches · monteurs · producteurs · DA · photographes · marques partenaires · événements · cercles créatifs</div><div style={{...mo,fontSize:10,fontWeight:800,color:t.d,letterSpacing:1.8,marginBottom:8}}>SIGNAUX</div>{["Progression rapide des vues","Vidéos solides à 30 jours","Communauté active","Commence à attirer les marques","Semble mal accompagné","Collabs faibles vs audience forte","Potentiel d'ambassadorat / co-branding"].map((x,j)=><div key={j} style={{...sa,fontSize:12,color:t.m,padding:"5px 0",lineHeight:1.4}}>· {x}</div>)}</>},{n:"02",t:"Qualification avant contact",render:()=><><div style={{...mo,fontSize:10,fontWeight:800,color:t.d,letterSpacing:1.8,marginBottom:8}}>FICHE COURTE</div>{["Vues moyennes YouTube 30j","Trois contenus représentatifs","Territoire éditorial","Audience supposée","Marques déjà faites","Marques qui pourraient matcher","Présence d'un agent / manager","Complémentarité avec le roster FAR","Raison précise pour laquelle FAR peut l'aider"].map((x,j)=><div key={j} style={{...sa,fontSize:12.5,color:t.c,padding:"7px 0",borderBottom:j<8?`1px solid ${t.brd}`:"none",lineHeight:1.4}}>{x}</div>)}</>},{n:"03",t:"Meilleure porte d'entrée",render:()=><><div style={{...mo,fontSize:10,fontWeight:800,color:t.d,letterSpacing:1.8,marginBottom:8}}>ORDRE DE PRÉFÉRENCE</div>{["Intro par quelqu'un de confiance","Intro par un talent FAR ou proche","Intro par un créatif, producteur, monteur ou DA","Contact via une marque partenaire","DM personnel et précis","Email professionnel si disponible"].map((x,j)=><div key={j} style={{display:"flex",alignItems:"baseline",gap:12,padding:"8px 0",borderBottom:j<5?`1px solid ${t.brd}`:"none"}}><span style={{...se,fontSize:18,fontWeight:900,color:j===0?t.a:t.d,lineHeight:1,minWidth:18}}>{j+1}</span><span style={{...sa,fontSize:12.5,color:t.c,lineHeight:1.4}}>{x}</span></div>)}</>}].map((col,i)=><div key={i} style={{padding:i===0?"0 28px 0 0":i===2?"0 0 0 28px":"0 28px",borderRight:i<2?`1px solid ${t.brd}`:"none"}}><div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:16}}><div style={{...se,fontSize:36,fontWeight:900,color:t.a,lineHeight:1}}>{col.n}</div><div style={{...sa,fontSize:15,fontWeight:800,color:t.c,lineHeight:1.2}}>{col.t}</div></div>{col.render()}</div>)}</div><div style={{marginTop:30,paddingTop:18,borderTop:`1px solid ${t.brd}`,display:"flex",gap:18,alignItems:"baseline"}}><div style={{width:36,height:2,background:t.a,flexShrink:0,position:"relative",top:-6}}/><div style={{...se,fontSize:20,fontWeight:800,color:t.c,fontStyle:"italic",lineHeight:1.4}}>Une approche courte, mais préparée. Une vraie lecture derrière, pas un recrutement au hasard.</div></div>{back&&<div style={{textAlign:"center",marginTop:20}}><button onClick={back} style={{background:t.nav,color:t.navT,...sa,fontSize:14,fontWeight:600,padding:"12px 32px",borderRadius:10,border:"none",cursor:"pointer"}}>← Retour à l'accueil</button></div>}</div>},
...
r:t=><div style={{display:"flex",gap:28,alignItems:"stretch"}}><div style={{flex:3,display:"flex",flexDirection:"column",justifyContent:"center"}}><Tg t={t}>PRÉSENTATION CYRIL</Tg><Hl t={t} s={{fontSize:36}}>CYRIL, un créateur multi-territoires.</Hl><Sh t={t}>CYRIL, propose sur sa chaîne des contenus très diversifiés mélangeant exploration, voyages, divertissement et gaming. Passionné et engagé, Cyril partage ses passions avec sa communauté depuis bientôt 15 ans.</Sh><div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:10,marginBottom:16}}>{[{v:"▶ 5,21M",l:"abonnés YouTube"},{v:"👥 8,5M+",l:"audience cumulée"},{v:"🧭 15 ans",l:"d'ancienneté créateur"},{v:"👁 872K",l:"moyenne de vues"}].map((s,i)=><div key={i} style={{padding:"12px 10px",borderRadius:10,background:t.card,border:`1px solid ${t.brd}`,textAlign:"center"}}><div style={{...se,fontSize:24,fontWeight:800,color:t.a}}>{s.v}</div><div style={{...sa,fontSize:11,color:t.m,marginTop:4,lineHeight:1.35}}>{s.l}</div></div>)}</div><div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:10}}>{[{v:"📊 36%",l:"18-24 ans"},{v:"📈 35%",l:"25-34 ans"},{v:"♂ 83,6% · ♀ 15,6%",l:"répartition genre"}].map((s,i)=><div key={i} style={{padding:"12px 10px",borderRadius:10,background:t.a+"14",border:`1px solid ${t.brd}`,textAlign:"center"}}><div style={{...se,fontSize:20,fontWeight:800,color:t.a}}>{s.v}</div><div style={{...sa,fontSize:11,color:t.m,marginTop:4,lineHeight:1.35}}>{s.l}</div></div>)}</div></div><div style={{flex:2,borderRadius:16,overflow:"hidden",flexShrink:0,border:`1px solid ${t.brd}`}}><img src={pu("/garmin-cyril-photo.png")} alt="CYRILmp4" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:16}}/></div></div>},

{title:"3 exemples de vidéos",
r:t=><div><Tg t={t}>UNIVERS ÉDITORIAL</Tg><Hl t={t} s={{fontSize:36}}>L'univers de Cyril : immersion, tension et spectacle.</Hl><Sh t={t} s={{fontSize:17,marginBottom:20}}>Exploration extrême, aventure spectaculaire et survie en terrain hostile : le ton de Cyril est fort, incarné et immédiatement identifiable.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>{[{img:"/garmin-cyril-thumb-1.png",t:"Exploration extrême",d:"Univers à risque, tension visuelle, narration immersive."},{img:"/garmin-cyril-thumb-2.png",t:"Aventure spectaculaire",d:"Promesse forte, décor hors norme, cliffhanger immédiat."},{img:"/garmin-cyril-thumb-3.png",t:"Survie & imprévus",d:"Contexte hostile, enjeu humain, storytelling de terrain."}].map((v,i)=><Wc key={i} t={t} s={{padding:10,border:`1px solid ${t.brd}`,boxShadow:"none"}}><img src={pu(v.img)} alt={v.t} style={{width:"100%",height:190,objectFit:"cover",borderRadius:10,marginBottom:10}}/><div style={{...se,fontSize:18,fontWeight:800,color:t.c,marginBottom:6}}>{v.t}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.45}}>{v.d}</div></Wc>)}</div><div style={{display:"flex",gap:10,marginTop:16}}>{[{t:"Fit éditorial",d:"Exploration, voyage, endurance, lieux extrêmes : des contextes où Garmin a une vraie raison d'être."},{t:"Usage visible",d:"Navigation, trace GPS, autonomie batterie, suivi cardio, altitude, météo, sécurité : des preuves concrètes dans le film."},{t:"Brand safety",d:"Un récit positif, maîtrisé et aspirationnel autour de la curiosité, de la préparation et du dépassement."}].map((a,i)=><div key={i} style={{flex:1,padding:"12px 12px",borderRadius:10,background:t.card,border:`1px solid ${t.brd}`}}><div style={{...sa,fontSize:13,fontWeight:700,color:t.a,marginBottom:4}}>{a.t}</div><div style={{...sa,fontSize:12,color:t.m,lineHeight:1.45}}>{a.d}</div></div>)}</div></div>},
...
// ═══════════════════════════════════════════════════════════════════════════════
// EDF × CYRILMP4  -  « ON MARCHE SUR UN RÉACTEUR NUCLÉAIRE »  -  13 SLIDES
// ═══════════════════════════════════════════════════════════════════════════════
const SEdf = [
{title:"Ouverture",
...
{title:"Insight",
r:t=><div><Tg t={t}>INSIGHT</Tg><Hl t={t} s={{fontSize:36}}>EDF possède exactement ce dont Cyril a besoin.</Hl><Wc t={t} s={{padding:"40px 36px",marginTop:24}}><div style={{...sa,fontSize:18,color:t.m,lineHeight:1.7,marginBottom:24}}>Des lieux interdits au public, spectaculaires et chargés d'histoire. L'intérieur d'une centrale nucléaire, les salles de contrôle, les turbines colossales : le terrain de jeu idéal pour son format documentaire.</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:24}}>{["Lieu interdit au public","Cadre spectaculaire","Charge historique","Salles de contrôle","Turbines colossales","Coulisses humaines"].map((p,i)=><span key={i} style={{...mo,fontSize:13,fontWeight:600,padding:"14px 20px",borderRadius:10,background:t.a+"15",color:t.a,textAlign:"center"}}>{p}</span>)}</div><div style={{...se,fontSize:22,lineHeight:1.4,color:t.a}}>Pour la première fois, le format signature de Cyril s'invite dans l'envers du décor d'une centrale en activité.</div></Wc></div>},
...
{title:"Positionnement",
r:t=><div><Tg t={t}>POSITIONNEMENT</Tg><Hl t={t} s={{fontSize:36}}>Une collaboration éditoriale authentique.</Hl><Sh t={t}>Quatre piliers structurants pour garantir un contenu natif, performant et long terme.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:24}}>{[{n:"01",t:"Pas de concept imposé",d:"Cyril reçoit un brief ouvert, adapté à sa ligne éditoriale. Le concept émerge de la collaboration, pas d'un script préécrit."},{n:"02",t:"Une immersion crédible",d:"EDF invite Cyril dans un de ses sites, hors normes. L'expérience est réelle, le cœur d'EDF comme terrain de jeu."},{n:"03",t:"L'intégration",d:"EDF apparaît comme partenaire de l'aventure, pas comme sujet principal. La marque rend possible, elle n'occupe pas le récit."},{n:"04",t:"Un brief évolutif",d:"On s'aligne avec Cyril en amont sur les messages clés. Le brief est adapté à sa personnalité pour garantir un contenu natif."}].map((p,i)=><div key={i} style={{padding:"26px",borderRadius:14,background:t.card,border:`1px solid ${t.brd}`}}><div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}><div style={{...mo,fontSize:11,fontWeight:700,color:t.a}}>{p.n}</div><div style={{flex:1,height:1,background:t.brd}}/></div><div style={{...se,fontSize:19,fontWeight:700,marginBottom:10,color:t.c}}>{p.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.6}}>{p.d}</div></div>)}</div></div>},

{title:"Phasing",
r:t=><div><Tg t={t}>PHASING</Tg><Hl t={t} s={{fontSize:34}}>De l'alignement créatif à la mise à l'antenne.</Hl><div style={{position:"relative",marginTop:40}}><div style={{position:"absolute",top:22,left:0,right:0,height:3,background:t.brd,zIndex:0}}/><div style={{display:"flex",gap:0,position:"relative",zIndex:1}}>{[{when:"T - 30",t:"Cadrage créatif",d:"Workshop FAR × EDF × Cyril, alignement messages clés. Choix du duo invité affinitaire.",edf:"Brief évolutif partagé, ligne éditoriale calée"},{when:"T - 15",t:"Préparation terrain",d:"Repérages Bugey, autorisations, sécurité, scénarisation des séquences.",edf:"Coordination équipes site, opérateurs identifiés"},{when:"JOUR J",t:"Tournage",d:"Immersion 2 à 3 jours dans la centrale, au plus près des équipes.",edf:"Accès zones rares, bâtiment réacteur en arrêt"},{when:"T + 45",t:"Mise en ligne",d:"YouTube long format, capsules courtes, relais stories. Coup d'envoi de la médiatisation.",edf:"YouTube Partnership Ads, paid social courts"}].map((phase,i)=><div key={i} style={{flex:1,paddingRight:i<3?16:0}}><div style={{width:44,height:44,borderRadius:"50%",background:t.a,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",...mo,fontSize:14,fontWeight:700,color:t.bg}}>{i+1}</div><div style={{padding:"20px 16px",borderRadius:12,background:t.card,border:`1px solid ${t.brd}`,minHeight:240}}><div style={{...mo,fontSize:12,fontWeight:600,letterSpacing:2,color:t.d,marginBottom:8}}>{phase.when}</div><div style={{...sa,fontSize:16,fontWeight:700,marginBottom:8}}>{phase.t}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.5,marginBottom:12}}>{phase.d}</div><div style={{padding:"10px 12px",borderRadius:8,background:t.a+"10"}}><div style={{...mo,fontSize:10,fontWeight:600,color:t.a,marginBottom:3}}>EDF VISIBLE</div><div style={{...sa,fontSize:12,color:t.m}}>{phase.edf}</div></div></div></div>)}</div></div></div>},

{title:"Dispositif",
r:t=>{const items=[{t:"Format long YouTube",d:"15 à 30 min, sur la chaîne",angle:-90},{t:"Capsules courtes",d:"Reels IG / TikTok / Shorts",angle:-30},{t:"Stories croisées",d:"Via créateurs de la saison",angle:30},{t:"Duo invité",d:"Affinitaire, choisi par Cyril",angle:90},{t:"YouTube Partnership Ads",d:"Diffusion sponsorisée par EDF",angle:150},{t:"Paid social courts",d:"Meta, TikTok · 15 sec",angle:210}];const R=200;return <div style={{textAlign:"center"}}><style>{`@keyframes _orbedf{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@keyframes _corbedf{from{transform:rotate(0deg)}to{transform:rotate(-360deg)}}._orbwedf{animation:_orbedf 22s linear infinite}._orbiedf{animation:_corbedf 22s linear infinite}._ecoedf._paused ._orbwedf{animation-play-state:paused}._ecoedf._paused ._orbiedf{animation-play-state:paused}`}</style><Tg t={t}>DISPOSITIF</Tg><Hl t={t} s={{fontSize:36,textAlign:"center"}}>Un écosystème de contenus, organique & médiatisé.</Hl><Sh t={t} s={{textAlign:"center",maxWidth:720,margin:"0 auto"}}>Chaque livrable a un rôle précis dans le récit et une fonction claire dans l'exploitation pour EDF.</Sh><div className="_ecoedf" style={{position:"relative",width:640,height:500,margin:"24px auto 0",cursor:"pointer"}}><div onMouseEnter={e=>e.currentTarget.closest("._ecoedf").classList.add("_paused")} onMouseLeave={e=>e.currentTarget.closest("._ecoedf").classList.remove("_paused")} style={{position:"absolute",left:"50%",top:"50%",transform:"translate(-50%,-50%)",width:140,height:140,borderRadius:"50%",background:t.a,display:"flex",alignItems:"center",justifyContent:"center",zIndex:2,cursor:"default",boxShadow:"0 0 60px rgba(254,80,0,.4)"}}><div style={{...se,fontSize:13,fontWeight:800,color:t.bg,textAlign:"center",lineHeight:1.2}}>RÉACTEUR<br/>BUGEY</div></div><div className="_orbwedf" style={{position:"absolute",left:0,top:0,width:"100%",height:"100%",transformOrigin:"320px 250px"}}>{items.map((it,i)=>{const rad=it.angle*Math.PI/180;const cx=320,cy=250;const x=cx+R*Math.cos(rad);const y=cy+R*Math.sin(rad);return <div key={i}><svg style={{position:"absolute",left:0,top:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:0}}><line x1={320} y1={250} x2={x} y2={y} stroke={t.brd} strokeWidth={1.5} strokeDasharray="4 4"/></svg><div className="_orbiedf" style={{position:"absolute",left:x-90,top:y-30,width:180,zIndex:1,textAlign:"center"}}><div style={{width:14,height:14,borderRadius:"50%",background:t.a,margin:"0 auto 6px",opacity:.7}}/><div style={{...sa,fontSize:14,fontWeight:700,marginBottom:2}}>{it.t}</div><div style={{...sa,fontSize:12,color:t.m}}>{it.d}</div></div></div>})}</div></div></div>}},
...
function DeckCard({id,st,d,onOpen,delay,children}) {
  const [hovered,setHovered] = useState(false);
  const logoVar = st.bg===B||st.bg==="#C62828"||st.bg==="#81B840"||st.bg==="#1A1A1A"||st.bg==="#050A0F"||st.bg==="#050C1A"?"white":"black";

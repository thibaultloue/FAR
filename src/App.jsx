...
/** FGC: fond pêche, rouge tomate, accent basilic, orange FAR sur c2. */
const TFGC = {
  bg:"#FFF5EC",
  c:"#1C1410",
  c2:A,
  ...
  card:"#FFFFFF",
  cardT:"#1C1410",
  cardAlt:"rgba(224,31,42,.07)",
  pill:"rgba(224,31,42,.11)",
...
const DeckMotif = ({deck}) => {
  ...
  if(deck==="fastgoodcuisine") return <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}}><defs><pattern id="dmfgc" width="40" height="40" patternUnits="userSpaceOnUse"><circle cx="10" cy="12" r="1.1" fill="rgba(224,31,42,.08)"/><circle cx="28" cy="26" r="0.9" fill="rgba(255,145,40,.14)"/><circle cx="22" cy="6" r="0.75" fill="rgba(15,107,88,.09)"/></pattern></defs><rect fill="url(#dmfgc)" width="100%" height="100%"/></svg>;
...
const EcoMap = ({t}) => <div style={{margin:"36px 0"}}>
  ...
  <div style={{display:"flex",alignItems:"center"}}>
    ...
    <motion.div initial={{scale:0}} animate={{scale:1}} transition={{delay:.15,type:"spring",stiffness:180,damping:14}} style={{width:160,height:160,borderRadius:"50%",overflow:"hidden",flexShrink:0,border:`3px solid ${t.a}`}}><img src="/lebouseuh.png" alt="Le Bouseuh" style={{width:"100%",height:"100%",objectFit:"cover"}}/></motion.div>
    <motion.div initial={{scaleX:0}} animate={{scaleX:1}} transition={{delay:.35,duration:.25}} style={{width:28,height:2,background:t.a+"30",flexShrink:0}}/>
...
{title:"Ouverture",
r:t=><div style={{display:"flex",gap:48,alignItems:"center",minHeight:"60vh"}}><div style={{flex:3}}><div style={{display:"flex",alignItems:"center",gap:20,marginBottom:32}}><img src="/lebouseuh.png" alt="Le Bouseuh" style={{width:64,height:64,borderRadius:"50%",objectFit:"cover",border:`3px solid ${t.brd}`}}/><div style={{width:1,height:36,background:t.brd}}/><FarLogo size={80} variant={t.lv}/></div><div style={{...se,fontSize:64,fontWeight:800,lineHeight:1,marginBottom:24}}>Le Bouseuh × Far</div><div style={{width:60,height:3,background:t.a,borderRadius:2,marginBottom:28}}/><div style={{...sa,fontSize:18,color:t.m,lineHeight:1.7,maxWidth:520}}>T'accompagner au quotidien.<br/>Développer tes actifs et tes projets.<br/>Structurer ton écosystème.<br/>Augmenter ta valeur.</div></div><div style={{flex:2,position:"relative"}}><img src="/case2_imgs/slide1_img1.jpg" alt="Le Bouseuh" style={{width:"100%",borderRadius:20,objectFit:"cover",aspectRatio:"4/5",filter:"grayscale(1)"}}/></div></div>},
...
// ═══════════════════════════════════════════════════════════════════════════════
// FASTGOODCUISINE  -  OP LE COMBAT DES CHEFS  -  LONG FORMAT YOUTUBE  -  7 SLIDES
// ═══════════════════════════════════════════════════════════════════════════════
const SFGC = [
{title:"Le concept",
r:t=><div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"65vh",textAlign:"center"}}><div style={{display:"flex",alignItems:"center",gap:24,marginBottom:36}}><FarLogo size={80} variant={t.lv}/><div style={{width:1,height:36,background:t.brd}}/><img src="/fgc.webp" alt="Fast Good Cuisine" style={{width:78,height:"auto",borderRadius:12,border:`1px solid ${t.brd}`,boxShadow:t.cS||"none"}}/></div><div style={{...mo,fontSize:12,fontWeight:600,padding:"8px 16px",background:t.pill,borderRadius:6,display:"inline-block",marginBottom:18}}>OP · LE COMBAT DES CHEFS</div><Hl t={t} s={{fontSize:42,textAlign:"center",maxWidth:940,margin:"0 auto 20px"}}>Le combat des chefs</Hl><div style={{...sa,fontSize:20,fontWeight:600,color:t.c,marginBottom:14,maxWidth:720}}>YouTube en long format : 50 chefs, jury star, un vainqueur.</div><div style={{...sa,fontSize:18,color:t.m,lineHeight:1.58,maxWidth:720,margin:"0 auto"}}>Un format FastGoodCuisine porté par un partenaire exclusif : compétition sur le plateau, rythme serré, pensé pour tenir jusqu'à la fin et vivre en replay.</div></div>},

{title:"Qui est Fast Good Cuisine",
r:t=><div style={{display:"flex",gap:32,alignItems:"stretch"}}><div style={{flex:3,display:"flex",flexDirection:"column",justifyContent:"center"}}><Tg t={t}>FAST GOOD CUISINE</Tg><Hl t={t} s={{fontSize:36}}>La chaîne food qui rassemble des millions de viewers.</Hl><Sh t={t}>Chaîne YouTube française majeure : recettes accessibles, défis et formats qui mettent la cuisine au centre. Une audience très large et engagée, le bon socle pour porter un long format événementiel avec jury et partenaire exclusif.</Sh><div style={{display:"flex",gap:12,marginBottom:24}}>{[{v:"7M+",l:"abonnés YouTube"},{v:"2,5 Md+",l:"vues cumulées"},{v:"2012",l:"création de la chaîne"}].map((s,i)=><div key={i} style={{flex:1,padding:"14px 12px",borderRadius:10,background:t.card,border:`1px solid ${t.brd}`,textAlign:"center"}}><div style={{...se,fontSize:26,fontWeight:800,color:t.a}}>{s.v}</div><div style={{...sa,fontSize:11,color:t.m,marginTop:4}}>{s.l}</div></div>)}</div>{[{t:"Chaîne",d:"Food grand public sur YouTube : longs formats, shorts et présence continue sur le fil."},{t:"Formats forts",d:"Recettes, défis, collaborations : la cuisine comme spectacle, lisible pour le sponsor."},{t:"Marques",d:"Territoires naturels alimentaire, équipement, retail et lifestyle autour de la table et du geste."}].map((a,i)=><div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:i<2?`1px solid ${t.brd}`:"none"}}><div style={{...sa,fontSize:14,fontWeight:700,color:t.a,minWidth:120}}>{a.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.5}}>{a.d}</div></div>)}</div><div style={{flex:2,borderRadius:16,overflow:"hidden",flexShrink:0}}><img src="/fgc.webp" alt="Fast Good Cuisine" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:16}}/></div></div>},

{title:"L'idée en une slide",
r:t=><div><Tg t={t}>L'IDÉE EN UNE SLIDE</Tg><Hl t={t} s={{fontSize:34}}>Le format en 4 blocs</Hl><Wc t={t} s={{padding:"30px 26px",marginTop:20}}><div style={{display:"flex",flexDirection:"column",gap:22}}>{[{n:"1",t:"YouTube, contenu phare",d:"Le rendez-vous principal sur la chaîne FastGoodCuisine : le récit avance dans la timeline de la vidéo, avec chapitres possibles pour faciliter la navigation et le replay."},{n:"2",t:"Les 50 chefs en compétition",d:"50 profils de cuisine réelle réunis sur le plateau : brigades, indépendants, profils techniques ou créatifs. Le plateau met en avant le geste, le goût, la pression du service, sur toute la durée du film."},{n:"3",t:"Le jury de chefs stars",d:"Des figures très populaires, ultra suivies sur YouTube et les réseaux. Notes, remarques, décisions serrées : leur présence crédibilise le niveau et pousse le référencement, les clips et les commentaires autour de la sortie."},{n:"4",t:"Un partenaire exclusif dans l'épreuve",d:"Ingrédient, matériel, prime cash, lieu ou twist scénique : une seule marque structure le jeu là où ça sert le plat ou le chrono. Chaque pic dans la vidéo devient un plan de coupe exploitable en court format après publication, toujours sous la même bannière."}].map((x,i)=><div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}><div style={{...se,fontSize:17,fontWeight:800,color:t.a2,minWidth:26}}>{x.n}</div><div><div style={{...sa,fontSize:16,fontWeight:700,color:t.c,marginBottom:6}}>{x.t}</div><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.6}}>{x.d}</div></div></div>)}</div></Wc></div>},

{title:"La mécanique",
r:t=><div><Tg t={t}>MÉCANIQUE</Tg><Hl t={t} s={{fontSize:36}}>Pression, élimination, progression.</Hl><Sh t={t}>Les chefs enchaînent des épreuves chronométrées devant le jury et les caméras. Le groupe se resserre au fil des segments jusqu'au climax final, avec un vainqueur.</Sh><div style={{display:"flex",gap:16,marginTop:26,flexWrap:"wrap"}}>{[{n:"01",t:"Corps du film",d:"Technique, créativité, binômes, panier mystère : enchaînements serrés pour garder la courbe de tension. Consignes lisibles pour le spectateur, angles de coupe pensés pour le montage d'un long format et pour les futurs extraits."},{n:"02",t:"Élimination",d:"Le jury de chefs stars commente, note, tranche. Duels, twists ou interventions publiques possibles. À chaque pivot du film, le spectateur sait qui monte et qui quitte le plateau."},{n:"03",t:"Climax",d:"Dernier service, couronne, vainqueur assumé sous les caméras. Le partenaire mis en avant sur la fin capte le pic d'attention (à la fin, reprises, commentaires). Suite logique : shorts, pubs sociales, retail, relances à partir de la master."}].map((x,i)=><div key={i} style={{flex:"1 1 220px",padding:22,borderRadius:14,background:t.card,border:`1px solid ${t.brd}`,boxShadow:t.cS||"none"}}><div style={{...mo,fontSize:11,fontWeight:700,color:t.a,marginBottom:10}}>{x.n}</div><div style={{...se,fontSize:17,fontWeight:800,marginBottom:8,color:t.c}}>{x.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>{x.d}</div></div>)}</div><div style={{marginTop:20,padding:"18px 24px",borderRadius:12,background:t.th2,border:`1px solid ${t.brd}`}}><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.62}}><span style={{fontWeight:700,color:t.th2T}}>Ce que ça produit</span> : une URL à pousser, une présence partenaire lisible le long de la timeline, des découpes Shorts et reels après coup, 50 visages de chefs et un jury star qui ancre la crédibilité culinaire.</div></div></div>},

{title:"Le ton",
r:t=><div><Tg t={t}>TON</Tg><Hl t={t} s={{fontSize:36}}>Spectaculaire, rythmé, massif.</Hl><Wc t={t} s={{padding:"36px 32px",marginTop:22}}><div style={{...sa,fontSize:18,color:t.m,lineHeight:1.75,marginBottom:22}}>Pensé pour le long format YouTube : lumière, son, plans serrés sur les mains et les visages, montage qui soutient la durée sans ramollir. La chaleur du plateau, la concentration des chefs, la tension quand le jury s'exprime, tout s'enchaîne dans un flux continu.</div><div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:18}}>{["Long format","Rebonds","Cuisine au centre"].map((p,i)=><span key={i} style={{...mo,fontSize:13,fontWeight:600,padding:"12px 16px",borderRadius:10,background:i===1?t.a2+"18":t.a+"14",color:i===1?t.a2:t.a}}>{p}</span>)}</div><div style={{...se,fontSize:21,lineHeight:1.45,color:t.a}}>La cuisine porte le spectacle du premier au dernier plan de la vidéo.</div></Wc></div>},
...
{title:"Signature",
r:(t,back)=><div style={{textAlign:"center",padding:"44px 0 28px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"58vh"}}><div style={{...mo,fontSize:12,fontWeight:700,letterSpacing:3,color:t.a,marginBottom:20}}>SIGNATURE</div><div style={{...se,fontSize:46,fontWeight:800,lineHeight:1.12,maxWidth:880,margin:"0 auto 22px"}}>50 chefs entrent.<br/>1 seul reste.</div><div style={{...sa,fontSize:17,color:t.m,lineHeight:1.65,maxWidth:580,margin:"0 auto 32px"}}>Phrase d'accroche pour clore le deck. Suite logique : tournage, montage de la master, casting des 50 chefs, booking du jury star, miniature et titre YouTube, intégration du partenaire exclusif sur la timeline, plan de sortie et de découpe sociale.</div><div style={{marginBottom:28}}><FarLogo size={84} variant={t.lv}/></div>{back&&<button onClick={back} style={{background:t.nav,color:t.navT,...sa,fontSize:14,fontWeight:600,padding:"12px 32px",borderRadius:10,border:"none",cursor:"pointer"}}>← Retour à l'accueil</button>}</div>},
];
...
r:t=><div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"65vh",textAlign:"center"}}><div style={{display:"flex",alignItems:"center",gap:24,marginBottom:36}}><FarLogo size={80} variant={t.lv}/><div style={{width:1,height:36,background:t.brd}}/><img src="/fgc.webp" alt="Fast Good Cuisine" style={{width:78,height:"auto",borderRadius:12,border:`1px solid ${t.brd}`,boxShadow:t.cS||"none"}}/></div><div style={{...mo,fontSize:12,fontWeight:600,padding:"8px 16px",background:t.pill,borderRadius:6,display:"inline-block",marginBottom:18}}>ACTIVATION · [MARQUE] × FASTGOODCUISINE</div><Hl t={t} s={{fontSize:42,textAlign:"center",maxWidth:940,margin:"0 auto 20px"}}>Le créateur food qui peut vendre bien plus qu'un placement</Hl><div style={{...sa,fontSize:20,fontWeight:600,color:t.c,marginBottom:14,maxWidth:720}}>[MARQUE] × FastGoodCuisine : média, divertissement et marques.</div><div style={{...sa,fontSize:18,color:t.m,lineHeight:1.58,maxWidth:720,margin:"0 auto"}}>Puissance média, divertissement grand public et prolongement business via Pepe Chicken & Pop's : une activation pensée comme un système, pas comme un simple placement.</div></div>},

{title:"FastGoodCuisine",
r:t=><div style={{display:"flex",gap:32,alignItems:"stretch"}}><div style={{flex:3,display:"flex",flexDirection:"column",justifyContent:"center"}}><Tg t={t}>PRÉSENTATION</Tg><Hl t={t} s={{fontSize:34}}>Un profil food rare : média, spectacle, business.</Hl><Sh t={t}>Trois leviers dans la même collaboration : puissance média, divertissement grand public, prolongement business via ses marques. Territoire éditorial clair : dégustation, divertissement, test/hack.</Sh><div style={{display:"flex",gap:12,marginBottom:20}}>{[{v:"7,5M+",l:"abonnés YouTube"},{v:"2M",l:"Instagram"},{v:"Formats",l:"tests, hacks, défis"}].map((s,i)=><div key={i} style={{flex:1,padding:"14px 12px",borderRadius:12,background:t.card,border:`1px solid ${t.brd}`,textAlign:"center",boxShadow:t.cS}}><div style={{...se,fontSize:24,fontWeight:800,color:t.a}}>{s.v}</div><div style={{...sa,fontSize:11,color:t.m,marginTop:4}}>{s.l}</div></div>)}</div><div style={{...sa,fontSize:15,color:t.m,lineHeight:1.58}}>Il rend le produit désirable et sait monter en spectacle. Ses formats naturels : comparatifs, tests, hacks, dégustations, grands concepts à enjeu. C'est ce qui lui permet d'aller du simple placement produit à des vidéos-événement comme Le Dernier Qui Quitte La Table ou Plat à 1€ vs 10 000€ avec MrBeast. Exemples publics à très fort volume de vues (ex. ~3,5 M sur une « Last to Leave the Table », ~2,7 M sur un food hack TikTok).</div><a href="https://www.youtube.com/@FastGoodCuisine" target="_blank" rel="noopener noreferrer" style={{...mo,fontSize:12,fontWeight:700,color:t.a,marginTop:16,textDecoration:"underline"}}>youtube.com/@FastGoodCuisine</a></div><div style={{flex:2,borderRadius:16,overflow:"hidden",flexShrink:0}}><img src="/fgc.webp" alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:16,minHeight:320}}/></div></div>},

{title:"Trois enjeux pour les marques",
r:t=><div><Tg t={t}>STRUCTURE</Tg><Hl t={t} s={{fontSize:32,marginBottom:8}}>Où jouer avec FastGoodCuisine</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginTop:22}}>{[{h:"Formats verticaux",d:"TikTok et Instagram : entrées les plus françaises, les plus tactiques. Premier palier de deals en volume, typiquement entre 5 k€ et 15 k€ : placements simples, démonstratifs, répétables. Développer les marques sur les formats courts : récurrence, démos, hacks. Snacking, grande distribution, livraison, FMCG du quotidien. Exemples de marques / canaux : Deliveroo, Uber Eats, Leclerc, Oreo, Nutella, Bonne Maman, Lindt, McCain…",c:t.a},{h:"Privilégier des vidéos dédiées sur YouTube",d:"YouTube : gros concepts et rayonnement large. Partenariats quali et intégrés sur des vidéos dédiées : la marque au cœur du récit. Électroménager cuisine : ex. Ninja / SharkNinja : blender/mixeur déjà fort en France, naturel pour recettes, hacks et démonstrations produits. Boissons et FMCG du quotidien s'y prêtent aussi sur le même principe de démo crédible.",c:t.a2},{h:"Pepe Chicken & Pop's",d:"Développer les actifs marque : collab menu, dessert, snack (Pepe Chicken) ; retail, VHR, sell-through (Pop's). Prolongement des verticales alimentaires et maison : co-branding, éditions limitées, présence linéaire. Cohérent avec snacking, grande distribution et livraison déjà actifs sur la chaîne.",c:t.a}].map((b,i)=><Wc key={i} t={t} s={{padding:20,marginTop:0,border:`2px solid ${t.brd}`,boxShadow:t.cS,borderTop:`4px solid ${b.c}`}}><div style={{...mo,fontSize:10,fontWeight:800,letterSpacing:2,color:b.c,marginBottom:10}}>{String(i+1).padStart(2,"0")}</div><div style={{...se,fontSize:16,fontWeight:800,color:t.c,marginBottom:10,lineHeight:1.2}}>{b.h}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.5}}>{b.d}</div></Wc>)}</div></div>},

{title:"Deux actifs, deux opportunités très différentes",
r:t=><div><Tg t={t}>PEPE CHICKEN · POP'S</Tg><Hl t={t} s={{fontSize:32}}>Au-delà de la chaîne seule</Hl><Sh t={t}>Pepe Chicken pour les collabs food très visibles ; Pop's pour pousser fort le retail et le hors domicile.</Sh><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:20}}><Wc t={t} s={{padding:22}}><div style={{...mo,fontSize:10,fontWeight:700,letterSpacing:2,color:t.a,marginBottom:10}}>PEPE CHICKEN</div><div style={{...se,fontSize:17,fontWeight:800,color:t.c,marginBottom:10}}>Collab food très visible</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Dessert, snack, add-on ; « dessert officiel » ou produit signature de fin de repas. Exemples de fit : Ben & Jerry's, Kinder Bueno, Oreo, Haribo, Lindt. Uber Eats / Deliveroo ; Taster indique Pepe Chicken dans plus de 110 villes en France et en Belgique.</div></Wc><Wc t={t} s={{padding:22}}><div style={{...mo,fontSize:10,fontWeight:700,letterSpacing:2,color:t.a2,marginBottom:10}}>POP'S</div><div style={{...se,fontSize:17,fontWeight:800,color:t.c,marginBottom:10}}>Retail & hors domicile</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.55}}>Thé glacé, fibres équivalentes à une pomme, vendu via Pepe Chicken et en grande distribution : Monoprix, Auchan, Franprix, Intermarché, Carrefour, U. Terrain pour collabs culturelles et commerciales type produit événementiel (réf. BK x KFC, BFF Burger, France fin 2024).</div></Wc></div></div>},

{title:"Le modèle recommandé",
r:(t,back)=><div><Tg t={t}>SETUP EN TROIS ÉTAGES</Tg><Hl t={t} s={{fontSize:32,marginBottom:14}}>Le bon setup pour FastGoodCuisine</Hl><div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:18}}>{[{n:"01",t:"Placements récurrents",d:"TikTok / Instagram / shorts pour faire du volume et monétiser les formats verticaux."},{n:"02",t:"Privilégier des vidéos dédiées sur YouTube",d:"Installer des prises de parole fortes qui renforcent le créateur et la marque."},{n:"03",t:"Ambition business",d:"Collabs contenu + produit + distribution autour de Pepe Chicken ou surtout de Pop's. Pepe Chicken : dessert / snack / menu collab ; Pop's : priorité retail, VHR, sell-through."}].map((x,i)=><div key={i} style={{display:"flex",gap:14,padding:16,borderRadius:12,background:t.card,border:`1px solid ${t.brd}`}}><div style={{...mo,fontSize:11,fontWeight:800,color:t.a,minWidth:28}}>{x.n}</div><div><div style={{...sa,fontSize:15,fontWeight:700,color:t.c}}>{x.t}</div><div style={{...sa,fontSize:14,color:t.m,lineHeight:1.5}}>{x.d}</div></div></div>)}</div><div style={{textAlign:"center",marginTop:28}}>{back&&<button onClick={back} style={{background:t.nav,color:t.navT,...sa,fontSize:14,fontWeight:600,padding:"12px 32px",borderRadius:10,border:"none",cursor:"pointer"}}>← Retour à l'accueil</button>}</div></div>},
];
...
function Home({onOpen}) {
  const cs = {
    ...
    fgc:{bg:"#FFF5EC",c:"#1C1410",tBg:"rgba(224,31,42,.11)",tC:"#B01822"},
      ...
      <div className="far-home-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:52,maxWidth:1155}}>
        {[{id:"case1",logo:"far"},{id:"case2",img:"/lebouseuh.png",imgType:"photo"}].map(({id,img,imgType,logo},idx)=>{const d=META[id];const st=cs[d.card];return(
          <DeckCard key={id} id={id} st={st} d={d} onOpen={onOpen} delay={.3+idx*.1}>
            ...
            {img&&imgType==="photo"&&<img src={img} alt="" style={{width:100,height:100,borderRadius:"50%",objectFit:"cover",border:"3px solid rgba(255,255,255,.2)",flexShrink:0}}/>}
          </DeckCard>
        );})}
      </div>
      ...
      <div className="far-home-grid" style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:20,maxWidth:1155}}>
        {[{id:"shopify",img:"/shopify-logo.png",imgW:110,filter:"brightness(0) invert(1)"},{id:"rode",img:"/rode-logo-white.png",imgW:80,filter:"none"},{id:"fastgoodcuisine",img:"/fgc.webp",imgW:76,filter:"none"},{id:"fgcmarque",img:"/fgc.webp",imgW:76,filter:"none"},{id:"toinelag",img:"/toinelag-avatar.png",imgW:78,filter:"none"},{id:"cyrilmp4",img:"/cyrilmp4.png",imgW:80,filter:"none",imgType:"photo"}].map(({id,img,imgW,filter,imgType},idx)=>{const d=META[id];const st=cs[d.card];return(
          <DeckCard key={id} id={id} st={st} d={d} onOpen={onOpen} delay={.5+idx*.1}>
            {imgType==="photo"?<img src={img} alt="" style={{width:imgW,height:imgW,borderRadius:"50%",objectFit:"cover",border:"3px solid rgba(255,255,255,.2)",flexShrink:0}}/>:<img src={img} alt="" style={{width:imgW,height:"auto",filter,opacity:.92,flexShrink:0,borderRadius:10,border: id==="toinelag" ? "3px solid rgba(20,20,20,.15)" : id==="fgcmarque" ? "2px solid rgba(244,240,234,.2)" : "none"}}/>}
          </DeckCard>
        );})}
      </div>

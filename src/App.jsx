...
r:t=><div><Tg t={t}>STRUCTURE</Tg><Hl t={t} s={{fontSize:32,marginBottom:8}}>Où jouer avec FastGoodCuisine</Hl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginTop:22}}>{[{h:"Formats verticaux & Shorts YouTube",d:"TikTok, Instagram et Shorts YouTube : formats courts pour montrer le produit, renouveler les messages et rester présent dans le fil.",c:t.a},{h:"Privilégier le dédié à l'intégration simple",d:"Une vidéo YouTube dédiée place la marque au centre du récit. C'est plus lisible et plus mémorable qu'un placement discret dans un format déjà tourné.",c:t.a2},{h:"Les actifs propriétaires",d:"Pepe Chicken : enseigne food (snack, menu, collab produit). Pop's : boisson retail, avec une logique grande surface et produit. Deux leviers au-delà de la seule chaîne YouTube.",c:t.a}].map((b,i)=><Wc key={i} t={t} s={{padding:20,marginTop:0,border:`2px solid ${t.brd}`,boxShadow:t.cS,borderTop:`4px solid ${b.c}`}}><div style={{...mo,fontSize:10,fontWeight:800,letterSpacing:2,color:b.c,marginBottom:10}}>{String(i+1).padStart(2,"0")}</div><div style={{...se,fontSize:16,fontWeight:800,color:t.c,marginBottom:10,lineHeight:1.2}}>{b.h}</div><div style={{...sa,fontSize:13,color:t.m,lineHeight:1.5}}>{b.d}</div></Wc>)}</div></div>},
